...
- Create: `src/evaluation/fitness/marketing_audit.py` (composite fitness: `weighted_rubric_score − cost_penalty × normalized_token_cost − latency_penalty × normalized_wall_clock`); writes computed fitness into `search_metrics.domains["marketing_audit"]["score"]` so existing `frontier.objective_score` `domain_score` branch picks it up (no third branch needed in `frontier.py:82-86`)
- Modify: `autoresearch/evaluate_variant.py` Stage 2 results writer — emit per-fixture inner_pass_rate / outer_pass_rate / pass_rate_delta into existing aggregator at `:1099-1132`; **drop the planned new `src/evaluation/fitness/correlation.py` module** (existing aggregator already does the math)
...
fitness = (
    weighted_rubric_score
  − 0.5 * normalized_token_cost     # cost_penalty: variant at 2× median cost loses 1.0 fitness
  − 0.3 * normalized_wall_clock      # latency_penalty: 2× median wall-clock loses 0.6 fitness
  + engagement_weight * engagement_signal  # 0.0 in v1; ramps to 0.5 max in v1.5
)
```
...
**Cost-penalty floor (per Key Decision §Live-wrapper contract):** `cost_penalty` term has a floor below which it stops contributing — preventing token-optimization from cannibalizing customer-facing deliverable quality. Specifically: `cost_penalty_effective = max(0, normalized_token_cost − 0.7)` so variants below 70% of median cost get no further fitness boost from cost reduction. A $5 token saving that degrades MA-6 polish is a bad trade on a $1K deliverable.
...
- Modify: `src/evaluation/structural.py:23-46` — extend `structural_gate` dispatch with `marketing_audit` elif branch routing to `_validate_marketing_audit` (defined in Unit 15)
...
- Create: `autoresearch/archive/current_runtime/programs/marketing_audit-evaluation-scope.yaml` (lane eval scope + output paths + transient paths per design §8.2)
...
Lane registration, engagement judge, critique manifest, pre-promotion smoke-test, autoresearch + claude CLI pins. **End of Phase 5 = full fusion plumbing wired + unit-tested; first end-to-end variant rotation in v1.5 once ≥1 marketing_audit holdout fixture lands** (per X.a fail-closed policy at line 159, locked as v1 default). v1 ships as a hand-promoted single-variant pipeline with all evolve infrastructure as plumbing — explicitly NOT a self-improving loop until smoke-test fixtures exist (post-audit-3 with prospect consent, per Open Question §Reader-readability + holdout-fixture sourcing). Bundle E.
