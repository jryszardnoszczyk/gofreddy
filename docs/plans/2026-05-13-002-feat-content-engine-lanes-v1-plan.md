...
- **TD-3.** HMAC signing key for U7 review URLs sourced from `GOFREDDY_REVIEW_HMAC_KEY` env var (never repo-checked-in); quarterly rotation runbook + dual-key overlap window; token TTL = per-client SLA; single-use via canonical-event idempotency check (per-client wide log scanned for an existing `review_approve` / `review_reject` event with matching `event_id` / `nonce` in metadata — see U7 + U6b).
...
- **TD-20.** U7 collapsed to 1–2 modules + 1 test file. Single `src/review/service.py` (~150–200 LOC: signed-URL helper, submit, decision handler, audit via `events.log_event`, SLA check) + `src/api/review_webhook.py` + single test file. Drop dedicated email module + storage module + SLA tiered-nag system. Audit emission goes through the canonical event log (no separate `audit.jsonl` file) per the 2026-05-18 portal-coordination pass — see U6b + U7.
...
- **TD-32 / 2026-05-15 substrate-finishing pass (verified-against-current-code).** Pre-content-design substrate audit. Verified 8 substrate claims against current code on main `bdc8068`; corrected 6. **Findings (Code Confirmed):** (i) `session_eval_registry.py` exists at `autoresearch/archive/v007-curated/workflows/session_eval_registry.py` with 6-lane `SESSION_EVAL_SPECS` dict ✓; (ii) `src/evaluation/models.py:160 EvaluateRequest.domain` is a 7-alternative `Literal[...]` ✓; (iii) `_assert_models_literal_matches()` at `autoresearch/lane_registry.py:584` is **callable, not module-load** — invoked from `evolve.py:2922-2923` + 2 test files (Item 4 failure-mode prose corrected); (iv) `RubricTemplate` dataclass at `src/evaluation/rubrics.py:16-27` has 6 fields — **`prose_ref` does NOT exist** despite TD-11+TD-30 plan claims (U5 now explicitly owns adding `prose_ref: str | None = None` + new `src/evaluation/rubric_resolver.py` module); (v) `evolve_ops.ensure_lane_heads` is a thin wrapper around `lane_runtime.initialize_current_heads` which writes manifest only, NOT workflow files (Item 6 mechanism corrected); (vi) cross-archive workflow file count = **14 propagation targets** (7 per-lane archives × {current_runtime, v007-curated}), not "~10" (Item 6 count corrected); (vii) `dimension_scores: []` empty-write at `evaluate_variant.py:1606, 1722, 1747` ✓; (viii) `asyncio.run(...)` precedent at `archive_x_engine/.../scripts/evaluate_session.py:497` ✓; (ix) **session-marker mechanism = `agent_retry.is_terminal_codex_failure()` stdout-pattern-match** at `agent_retry.py:144-156` (NOT a marker file; U13/U14/U15/U15b prose + new checklist Item 11 corrected to reference `compute_metrics.append_generation_row()` writer at `compute_metrics.py:138-141`); (x) **RND-1..RND-5 are NOT in `RUBRICS`** — they're parallel rubrics inline in per-archive `render_judge.py` scripts (Item 5 prose corrected; 92-total RUBRICS count remains accurate). 6 surgical edits applied to the plan; no design decisions changed; no implementation work started. Scope: substrate-only verification preceding the upcoming content-design + data-provider research passes.

- **TD-33 / 2026-05-15 generic-first generalization (SaaS/AI primary mix).** JR direction: stop overfitting design decisions to Klinika+DWF specifics. Expected v1.5+ majority = SaaS tools + AI-first companies (`b2b_saas`, `b2b_ai_first` archetypes); Klinika/DWF are deliberately-diverse first onboarded clients, not the design target. §Generalization Justification rewritten to name the expected mix, archetype enum expanded to 7 named archetypes, `ClientConfig.locale` field added (per-client, not Polish-hardcoded; SaaS/AI default `en-US`), `ClientConfig.voice_corpus_consent_required` field added (per-client, archetype-derived defaults), `ClientConfig.brand_strictness` field added (default `strict`; `permissive` for verticals without mature brand assets). Parallel-track risk #1 reframed as per-client property. SE-8 anti-slop authority confirmed for SaaS/AI mix (calibrated from gofreddy.ai, an AI-first SaaS site); per-client rubric softening pattern documented in §Generalization Justification for non-tech verticals. `ai_disclosure_eu` (EU AI Act Art. 50) + `ai_disclosure_us` (FTC AI advertising) compliance rule sets added to "Architecture supports but build deferred." Voice corpus loader (U3) extended to handle HTML + JSON + markdown + PDF formats (SaaS clients typically have public blog archives + Twitter/LinkedIn data exports).

- **TD-34 / 2026-05-15 ad-creative data provider decision.** Ad-intelligence stack = 3 providers operating in parallel (existing Foreplay + Adyntel subscriptions retained per JR direction; Meta Ad Library API added as free official cross-check). Merger logic per Decision 1 research: dedupe by ad_id (Meta) or content-hash; source confidence weights (Meta Ad Library 1.0 / Adyntel 0.85 / Foreplay 0.75); cross-validation bonus `× 1.3` for ads in ≥2 sources; freshness floor 90 days (mitigates Foreplay's archive-depth weakness); geo gating by `target_audience.geo` from client config; format-match prioritization. `signal_merger.py` (new) implements this; signal_aggregator runs all 3 in parallel via `asyncio.run`. Foreplay rejection rationale generalizes (not Polish-specific): crowdsourced ingestion (Chrome extension users) produces sparse coverage for any non-US-DTC vertical; rejection avoided here by JR's "use what we already have" pragmatism. Adyntel = `$44/mo (5K credits)`; Foreplay subscription pre-existing; Meta Ad Library API free + 200 calls/user-token/hour. Cache TTLs in §Operational Notes table.

- **TD-35 / 2026-05-15 SERP + first-party SEO providers.** SERP signal = SerpAPI Starter `$25/mo (1K queries)` at v1 volume (2 clients × 5 runs/wk × ~15 queries = 600 queries/mo with 40% headroom). First-party SEO = Google Search Console API (free). Architecture: **shared OAuth client** = one gofreddy Google Cloud project + per-client refresh tokens encrypted, scoped to `client_id` (NOT per-client Cloud project). Critical onboarding gotchas documented in §Operational Notes: OAuth consent verification 2-6 weeks (sensitive scope `webmasters.readonly` — submit at FIRST client onboarding), refresh tokens expire after 6 months unused (weekly cron refresh), "Testing" status = 7-day refresh tokens (verification must complete before first production client), small B2B clients across verticals often don't have GSC set up (operator-assisted DNS-verified Domain property adds 1-2 hours per client). Switch trigger to cheaper alternatives (Serper.dev `$0.30-1.00/1K` or DataForSEO Standard `$0.60/1K`) at >5K queries/month — meaningful only at ~3rd client. Locale handling moves to per-client config (TD-33); SerpAPI requires all 3 locale params (`gl/hl/google_domain`).

- **TD-36 / 2026-05-15 article_engine source material acquisition.** Source material directory = `clients/<slug>/source_material/` (operator-curated handoff folder). Paths referenced from `ClientConfig.source_material_paths`. Formats supported: markdown preferred (most natural for LLM); PDF via existing `src/audit/pdf_to_markdown.py`; HTML via BeautifulSoup; URL list via existing `src/competitive/providers/` patterns. Uses existing `session_eval_<lane>.load_source_data(mode, artifact, session_dir)` primitive (precedent at `archive/v038/workflows/session_eval_competitive.py:115`). **AE-3 citation handling for v1:** counts `[N]` references in output (citation density); does NOT fetch URLs to verify claims (deferred to v1.5). Defensible because pre-publish human review (U7) catches unverified claims and SaaS/AI clients typically use self-referential citations. Re-add trigger: regulated SaaS client (fintech, healthtech) onboards OR pre-publish reviewer SLA breach >2× per week on AE-3-related rejections.

- **TD-37 / 2026-05-15 brand asset acquisition.** Per-client brand directory at `clients/<slug>/brand/` with structured assets: `logo.svg` (preferred) or `.png` (fallback); `fonts/*.woff2` (100% local — no external CDN per threat model); `tokens.json` (color palette + typography + spacing scale, Pydantic-validated per threat model: 64KB max / depth 8 / string 1024 / no `https?://` URLs). **Strictness flipped to `strict` by default** per JR direction (SaaS/AI clients have mature brand assets — SVG logo + WOFF2 fonts; preflight fails on miss). Permissive fallback (`brand_strictness = "permissive"`) allows PNG logo + system-font for verticals without mature brand assets (warns instead of fails); Klinika+DWF may set `permissive` at onboarding. Resolved via env var (`IMAGE_ENGINE_BRAND_TOKENS_PATH` for image_engine; `site_engine.brand_tokens` from client.yaml for site_engine). CODEOWNERS protects `clients/*/brand/` per existing TD-5 pattern.

- **TD-38 / 2026-05-15 fixture authoring strategy.** **Real_client fixtures live out-of-repo** at `~/.config/gofreddy/holdouts/<archetype>/<lane>/` (matches existing holdout pattern at `autoresearch/eval_suites/SCHEMA.md:80` — chmod 600, loaded via `EVOLUTION_HOLDOUT_MANIFEST` env). **Synthetic + stub fixtures stay in-repo** at `autoresearch/eval_suites/`. For clients without prior Content Engine output (DWF, most new SaaS clients): real_client = **approved past published work** (LinkedIn posts, blog archives, newsletter copy, partner content) — client signs off as representative. Effort estimate: **2-3 hours active operator time per (archetype × lane)** = ~20 hours total per onboarded client across 4 lanes at v1 launch. SaaS/AI clients with active public content programs make fixture authoring EASIER than for Klinika+DWF (lots of pre-published material to draw from). D11 `data_provenance: Literal["real_client", "synthetic", "stub"]` field is net-new substrate added in U1 or U2 (currently not on `EvaluationFixture` model).

- **TD-39 / 2026-05-15 cache / freshness defaults per source.** Cache TTL table added to §Documentation / Operational Notes (Foreplay 24h live / 7d historical; Adyntel 24h live / 7d historical; Meta Ad Library 12h; SerpAPI 24h; GSC 24h with daily 06:00 UTC refresh; briefs per-source 24h/7d/14d; voice corpus + brand_tokens snapshot at run-start with fail-loud on drift per D7). Per-provider TTL constants land in `src/ads/signal_aggregator.py` + `src/briefs/reader.py`. `--fresh` CLI flag bypasses provider-level cache for ad-hoc reruns.

- **TD-40 / 2026-05-15 article_engine internal design (U13).** Workflow shape: **single parametrised `WorkflowSpec` dispatched by `platform_target`** ({blog, linkedin_article}); platform-specific deltas are post-write adapters, not separate workflows (mirrors x_engine's `length_bracket` parametrisation precedent). **Brief consumption: top-K=3 hybrid** — ONE designated `primary_brief` (drives thesis + voice register + hook) + 2 `evidence_briefs` (mined for stats, named entities); session prompt forces "every numeric claim must trace to a brief.source_id." **Length conventions: SaaS/AI blog 1,500–2,500 words target 1,800; LinkedIn Article 1,200–1,800 target 1,500; hard caps blog 800/4,000 + LI 600/2,200 (out-of-bounds = structural fail).** **Mutation surface:** `programs/article_engine-session.md` + `templates/article_engine/skeleton-*.md` + decomposed stage prompts; workflow Python + rubric prose + voice references + compliance rules all read-only. **AE-1..AE-8 anchor draft (refined):** AE-1 hook strength / AE-2 thesis specificity & falsifiability / AE-3 citation density & verifiability / AE-4 voice fidelity / AE-5 argument coherence & structure / AE-6 skimmability & rhythm / AE-7 platform-adapter compliance / AE-8 cross-cohort diversity & novelty. Original "specificity" split into thesis-level (AE-2) + citation-level (AE-3) because they fail independently in AI-generated content. **12 enforced anti-patterns:** "In today's fast-paced world" opener; listicle-as-default structure; "studies show" without inline source; self-promoting "best of" comparisons; AI register vocab (seamlessly, robust, holistic, leverage, optimize, streamline, cutting-edge, game-changing, revolutionary, transform); hedge stack within 3 sentences; academic transitions density >2× per 1K words; padded intro before the point; unmotivated rhetorical question opener; footnoted-but-untrue citations; bait-y CTA close; restated-thesis conclusion. **Enforcement:** deterministic regex-and-token-match pre-check (banned phrase list in `templates/article_engine/anti_patterns.yml`) caps AE-1 score at 4 on hit; rubric scores capture the qualitative residual.

- **TD-41 / 2026-05-15 image_engine internal design (U14).** Workflow shape: **single workflow with format-dispatch**; 6 formats share ~70% of pipeline; `composition.py` exports `compose(format, slides, brand_tokens) -> [PNG]` and dispatches internally. **fal.ai prompt-iteration loop: anchored templates + 1-shot critique pass** (hybrid). Maintain `templates/image_engine/fal_prompts/<format>/<archetype>.md` (e.g., `ig_single/product-spotlight.md`, `li_doc_carousel/data-callout.md`); agent picks template by archetype-classification → fills slots → ONE internal critique pass against stripped 3-rubric subset (IE-1/2/5) → send to fal. No image regen (D21 preserved). **Carousel storytelling: two-step** — outline pass (claude/opus emits `[{slide_n, role, headline, key_visual_concept}]` with PSR or hook-stakes-value(×N)-proof-cta arc; force PSR for `li_doc_carousel`) → per-slide fal-prompt pass (parallel batches of 3, each receives previous-slide visual *summary* — factual recap, not the image — for palette/composition coherence without serial blocking). Cover slide gets dedicated archetype template (bold-claim + data-point + swipe-indicator). Slide-count enforcement: hard-fail `li_doc_carousel <7 or >12` and `ig_carousel <5 or >10` at L1 structural. **Brand-stamp: hybrid baked-in vs composited.** Palette + lighting register + typography *style* hint baked into fal prompt (via hex codes — Flux 2 handles this); exact logo PNG + exact-text headlines/CTAs/slide-numbers Pillow-composited after. Never let fal render brand wordmarks (hallucination produces extra-fingers-in-logo failure modes). **Vision-judge prompt: multi-dimensional rubric prompt** with hex-coded brand tokens (not swatch images), tight context payload, per-rubric dimensions enumerated. Per-rubric dimensions: IE-1 `stop_scroll_strength/focal_clarity/thumbnail_legibility`; IE-2 `palette_fidelity/typography_consistency/logo_treatment/iconography_register`; IE-3 `legibility_at_thumbnail/whitespace_balance/hierarchy_clarity`; IE-5 `concept_concreteness/absence_of_generic_filler/metaphor_strength`; IE-6 `cover_hook/slide_pacing/payoff_strength/cta_clarity` (carousel only). Vision-judge fires per-image; carousel rolls up via `mean(dimension_scores) + min(score) gate`. **IE-1..IE-8 anchor draft:** IE-1 hook visual / IE-2 brand consistency / IE-3 info density legibility / IE-4 format compliance / IE-5 visual specificity / IE-6 carousel arc / IE-7 alt-text + voice-consistent caption / IE-8 repurposability. **Mutation surface:** `programs/session/image_engine.md` + `templates/image_engine/skeletons/<format>.md` + `templates/image_engine/fal_prompts/<format>/<archetype>.md` (highest-leverage surface — most evolution wins land here) + `scripts/image_composer.py` composition functions + `templates/image_engine/anti_patterns.yml`. Read-only: brand_tokens (client-controlled), rubric anchor prose, hard format specs, vision-judge prompt structure. **15 enforced anti-patterns** (deterministic pre-check + vision-judge `failure_modes_observed` non-empty caps IE-5 at 4): lime+purple+dark gradient palette; three-icon trio with generic gradients (ported from SE-8); floating 3D shapes; stock open-concept office workers; glassmorphism/neumorphism cards; hallucinated logos/wordmarks; hand artifacts (extra fingers); garbled image-internal text; cover-identical-to-interior carousel; wall-of-text slide (>60 words on `li_doc_carousel` / >15% pixel-area text on ad); generic abstract metaphor; Inter-default typography when brand specifies otherwise; B2B stock cliché (hero-image diverse-group-laughing-at-laptop); emoji-in-B2B for `li_doc_carousel`/enterprise `hero_banner`; off-palette accent colors (ΔE >15 from brand tokens).

- **TD-42 / 2026-05-15 ad_engine internal design (U15).** Workflow shape: **hybrid parallel-skeleton + sequential-body variant generation.** Parallel skeleton pass (one cheap call: "produce 5 distinct angles for this ad — hook archetype ∈ {statistic, pain, contrarian, demo-tease, pattern-break} + promise sentence + single proof noun") forces angle-diversity at lowest token cost. Sequential body pass (one call per variant in N=4 loop, each conditioned on assigned skeleton + prior variants' full text): "produce the full ad creative + LP hero for skeleton K. Do not reuse the opening N-gram, structural cadence, or proof noun of variants {1..K-1}." Matches Andromeda + Motion findings: angle diversity > wording diversity. **Diversity gate behavior:** reject + regen failing slot with budget=2 retries; on third fail, persist `variant_k = null` + `failure_reason = "diversity_gate"`; per-format minimum: if fewer than `ceil(N/2)` variants survive, fail the platform-format + surface meta-failure. **Signal consumption pattern (Option D hybrid):** `signal_aggregator` pre-step (pure Python, no LLM except final summarization) produces structured `creative_brief.json` — `top_competitor_ads` (top-N=8 ranked by longevity × cross-source-confidence × format-match) + `recurring_hook_archetypes` (clustered with frequency counts) + `serp_signal` (top-5) + `gsc_signal` (top-10 28-day delta) + `competitor_voice_anti_examples` (3-5 hooks to NOT mimic). Plus ONE LLM summarization call → 6-10 bullet "what's working / saturated / opening" prose. Main agent consumes structured brief + prose summary; raw signal stays in annex. **Platform-format variant matrix LOCKED at 4/4/4/3 with per-format override via client.yaml:** Meta Reels Ad 4 (vertical 9:16 1080×1920, 9-15s, hook in first 0.8-1.2s, ≤8-word on-screen hook; diversity dim = hook archetype); Meta Image Ad 4 (1:1 1080×1080 primary + 4:5 1080×1350 secondary; ≤125 char primary text; no stock photos; diversity dim = promise type); LinkedIn Sponsored Content 4 (≤150 char intro, first-person voice, custom graphics — 0% of top-2%-CTR LinkedIn ads use stock; diversity dim = insight angle); LinkedIn Document Ad 3 (5-7 slides; cover works standalone; diversity dim = content shape). **Compliance enforcement TIERED:** hard structural gate for platform-policy violations that WILL get rejected (Meta health-vertical word avoidance `cure|treat|heal|fix|diagnose|symptoms` for `vertical=health|wellness|aesthetic_medicine`; before/after imagery for health verticals; LinkedIn `guaranteed ROI|secret hack|instant hire|spam|aggressive`; "Guaranteed [N]% results"); soft rubric for stylistic-fit (LI B2B reading consumer-y, headline > recommended length, >20% text-on-image). **Creative + LP copy linkage: same-agent single-pass** producing both. Hard structural gate: `jaccard(tokenize(ad.hook), tokenize(lp.headline)) ≥ 0.4` after stopword removal (message-match research: 2.3% conversion lift per 1% headline alignment); `lp.cta.verb == ad.cta.verb` enforced; `ad.body.proof_noun ∈ lp.proof_point`. **AD-1..AD-8 anchor draft (operationalized):** AD-1 hook strength / AD-2 CTA clarity / AD-3 offer specificity / AD-4 platform-format compliance / AD-5 variant diversity / AD-6 voice fidelity / AD-7 market-signal alignment (R19 no-op when ALL 3 Meta-side providers degrade simultaneously) / AD-8 conversion-readiness. **Mutation surface:** `programs/session/ad_engine.md` (main + diversity-pass + brief-synthesizer prompts) + `templates/ad_engine/skeleton-*.md` per format + `src/ads/signal_aggregator/weights.yaml` (merge confidence weights from TD-34) + `src/ads/signal_aggregator/brief_prompt.md` (LLM summarization call) + `compliance/anti_patterns.yaml`. Read-only: workflow.py, signal_aggregator deterministic logic (dedupe/hash/geo gating — per Rule 5 code-not-LLM), rubric anchors. **14 enforced anti-patterns** (deterministic pre-check in `src/ads/compliance/anti_patterns.py`, regex + token match, no LLM; hit caps AD-1 at 4 + AD-6 at 3): "Tired of X? Meet Y" PAS opener; "Unlock"; "Harness the power of"; "Revolutionize"; "AI-powered" without specific capability noun in same sentence; "Leverage"; "Seamlessly integrate"; "Game-changer / next-generation / cutting-edge"; generic `Learn More` CTA; stock office-worker-at-laptop OR people-shaking-hands; founder studio-shot photo with "the future of X"; three-icon trio (already SE-8); "Built for modern teams"; headline >12 words.

- **TD-43 / 2026-05-15 site_engine internal design (U15b).** **Canonical sub-elements per section** (agent-visible fields, structural-gate fails on missing required): `hero` req `{h1, subhead, primary_cta}` + opt `{eyebrow, secondary_cta, visual, trust_strip, friction_reducer}`; `value_prop` req `{section_heading, body}` + opt one of `{three_card_grid, feature_bullets, alternating_feature_blocks}` (**SaaS/AI clients prefer `alternating_feature_blocks` Linear/Stripe-pattern; three-card grid is a slop signal**) + opt `{subhead, inline_visual}`; `social_proof` req one of `{logo_wall, testimonial_cards, metric_callouts, rating_badge}` + combinations allowed; `faq` req `{section_heading, qa_pairs (5-10), accordion_default_open_first: bool}`; default `<details>` native accordion + JSON-LD FAQ schema by default for SEO; `cta` req `{heading, primary_cta}` + opt `{body, secondary_cta, friction_reducer, inline_avatar_proof}`; `pricing` req `{tier_cards (2-4, ideally 3, one with recommended: true — pages without highlighted tier convert ~22% worse), billing_toggle_default}` + opt `{feature_comparison_table, enterprise_callout, disclaimer, recommended_tier_index}`. Tier card sub-elements: `tier_name, price, price_unit, description, feature_list (5-9 bullets), cta, optional badge`. **Mutation surface — 3-tier whitelist** declared in `LaneSpec.mutation_surface`. **Tier-A always allowed (~85% of useful evolution):** all text content within any sub-element; add/remove **optional** sub-elements declared per section; swap variant within `{accept-one-of}` group (logo_wall ↔ testimonial_cards ↔ metric_callouts); CSS values *resolved through brand_tokens* (pick different spacing token `--space-4 → --space-6`, different palette role `bg.surface → bg.surface-elevated`, different type-scale step); component-count tweaks within sane bounds (3-card → 4-card grid; 5 → 7 FAQ items; pricing 2 → 3 tiers). **Tier-B allowed with structural-gate scrutiny:** layout structure swap from declared `layout_recipes` list per section (e.g., hero gets 3 named layouts: left-text/right-image ↔ centered ↔ split-screen). Free-form layout invention is out. **Tier-C forbidden v1:** token-value mutation (no inventing new colors/fonts/spacing — JR's READ-ONLY constraint on brand_tokens); new `<script>` outside allowlist; cross-section composition; required sub-element removal. **Brief consumption: 1 finding → N=4 variants**, each with different `mutation_strategy` from fixed enum: `{copy_rewrite, hierarchy_reweight, layout_swap, sub_element_add, sub_element_remove}`. Diversity-in-strategy avoids 4 variants that all just rewrite the headline. Multi-brief synthesis deferred to v1.5 (known coupling failure mode). **Variant count: 4 per section per finding, hard cap 12 total per run** (reviewer triage ~3min/variant set = ~9min/run). Per-section `max_variants_per_run` override allows budget-constrained clients to drop to 2. **Reviewer-feedback capture (v1 manual quarterly review process) — wire data substrate NOW:** BOTH structured + free-form. Structured = nightly diff job runs textual + DOM diff on `(approved_html, lane_output_html)` pair and tags edit categories: `copy_change, tone_change, sub_element_added, sub_element_removed, layout_swap, token_role_swap` (categories predefined; uncategorized → `manual_review` bucket). Free-form = required `reviewer_note` field (≥1 sentence) explaining why edit was needed — this is the calibration gold; empty notes → CI warning. **Quarterly review session (2-hour operator workflow):** 15min read auto-digest (accept/edit/reject rate per SE-axis + top 10 edit categories + top 5 LLM-clustered reviewer-note themes) → 45min anchor walk for SE-axes with edit-rate >30% OR judge-vs-reviewer disagreement >1.0 points → 30min author revisions to `docs/rubrics/site-quality.md` (material changes bump `version`) → 30min calibration smoke (re-score 10 historical fixtures; if Spearman correlation with reviewer ground truth doesn't improve, revert). Artifacts: versioned `site-quality.md` v1→v2, append-only `docs/rubrics/site-quality-calibration-log.md` (audit trail per anchor change), refreshed `fixtures/site_engine/calibration/` set. Ad-hoc trigger conditions (don't wait for quarterly): new client onboarded with materially different vertical; reject rate on any lane >50% for 2-week window; 3 consecutive weekly digests flag same edit category; material judge model swap. **SE-1..SE-8 authority CONFIRMED for SaaS/AI mix** (calibrated from gofreddy.ai, an AI-first SaaS company). **Per-client softening pattern for non-tech verticals (Klinika, DWF):** `rubric_overrides: site_engine: SE-1: {cta_prominence_target: "restrained"} (legal B2B doesn't want screaming CTAs), SE-2: {domain_vocabulary: ["thread lift", "hyaluronic"], jargon_threshold_words: 12} (medical clarity ≠ tech clarity), SE-3: {claim_compliance_ruleset: "medical_pl", numeric_claim_backing_required: true}, SE-8: {slop_signatures: ["stock_lab_coat_photo", "rose_gold_gradient"]}`. Overrides surface in judge prompts via templating; default = no override = SE-* anchors as-written. **10 additional SaaS/AI anti-patterns to fold into SE-8 v2** (beyond current authored anchors): "Build the future of X" / "All-in-one platform for X" headline patterns; Inter font on everything; **colored left-border accent card** (single most reliable AI-design tell); "Powered by AI" / "AI-first" badge without specification; anthropomorphic agent language (autonomous, intelligent, thinking, understands, knows, decides); founder selfie hero illustration; generic placeholder dashboard data ("$1,234,567 MRR", "John Doe"); bento-grid feature layout when features don't actually vary in importance; 16px uniform border-radius on every element; centered everything with no asymmetry.

- **TD-44 / 2026-05-15 AE-3 citation verification builds in v1 (NO deferral).** JR rejected the "defer to v1.5" framing. **AE-3 verification ships in v1 build.** Implementation: new module `src/verification/citation_verifier.py` (~150 LOC). **Behavior:** fires only on claims with URL citations (claims without URLs = hard structural fail; `[N]` reference must point to a `source_id` with URL); per-claim flow: fetch URL via `httpx.AsyncClient` → extract main content via BeautifulSoup → claude/opus call "does this URL substantiate claim X" → returns `{verified: bool, confidence: float, rationale: str}`. **Caching:** verified `claim+URL` pair cached indefinitely (URL content rarely changes meaningfully); `--fresh` CLI flag busts cache for ad-hoc re-verification. **Failure modes** (URL 404 / paywalled / JS-heavy site that doesn't render in scraper): mark `citation_verification_degraded` on variant + flag for human reviewer. **AE-3 rubric scores BOTH dimensions:** density (count of `[N]` references per 1,000 words) AND verification rate (% of claims with `verified: true`). **Cost envelope:** ~$0.01-0.03 per claim verification (claude/opus call); average article has 3-8 citations → ~$0.10/article. **Latency:** cached path free; uncached ~3-5s per claim → ~30s for 8 unverified-yet claims (acceptable; offline-cacheable). **Files added to U13:** `src/verification/citation_verifier.py` (new module) + `src/verification/citation_cache.py` (cache layer) + `tests/verification/test_citation_verifier.py` + `tests/verification/test_citation_cache.py`. **Rejected rationale for deferral:** production-grade v1 + no demo deadline argues for safer build at start; retrofit cost > build-now cost (rubric anchor recalibration when verification lands later changes AE-3 score distributions); risk-reduction meaningful beyond regulated verticals — SaaS/AI thought-leadership cites industry data constantly and fabricated stats are real brand-damage failure mode.

- **TD-45 / 2026-05-16 video generation: scripts + prompts only, manual fal.ai I2V step (per JR direction).** JR rejected automated video generation in v1 — too expensive, don't want autoresearch-evolution loops burning tokens on multi-second video gens. **Lane outputs are "video plans," not video binaries.** ad_engine Meta Reels Ad output = self-contained Reels Ad plan: hook copy + scene-by-scene script + image briefs for keyframes (image_engine fulfills cheaply) + voiceover script (15-30 words) + on-screen text overlays + CTA. Operator runs fal.ai I2V manually using the plan. Storyboard lane (U8) stays independent for storyboard-only outputs (educational videos, explainers — also script-only, no automated video binary generation in v1). **Cost-saving rationale:** fal.ai I2V is ~$0.30-1.50 per 5-10s clip; 4 Reels Ad variants × 5 fixtures × 10 generations/evolution-iteration = $60-300 burned PER VARIANT EVOLUTION CYCLE. Manual generation post-promotion bounds video cost to ~$10-50/onboarded-client/month vs. $1000s for fully-automated. v1.5 reconsiders if a client contracts for high-volume video output that justifies the spend. **Plan impact:** U15 ad_engine "Reels Ad plan" output schema documented; U8 storyboard prose clarified (scripts-only, no binary gen in v1); §Scope Boundaries adds "No automated fal.ai I2V in v1; lanes produce video plans, operator runs generation manually."

- **TD-46 / 2026-05-16 GDPR EU + AI disclosure rule sets BUILD in v1 (NOT deferred).** Per JR direction: gdpr_eu is important from the beginning given SaaS/AI primary mix. **Authored in v1 build alongside `_placeholder_medical_pl.yaml` + `_placeholder_legal_pl.yaml`:** (a) `compliance/rule_sets/gdpr_eu.yaml` — baseline for any EU-targeting client; data-handling claims (cookie disclosures, retention, right-to-erasure, lawful basis representations); ~80-150 rules from public GDPR articles. (b) `compliance/rule_sets/ai_disclosure_eu.yaml` — EU AI Act Art. 50 transparency obligations (AI-generated content must be marked detectable; deepfake labeling; emotional-recognition system disclosure); ~30-50 rules from EU AI Act text. (c) `compliance/rule_sets/ai_disclosure_us.yaml` — FTC AI advertising guidance (no false AI capability claims; no undisclosed automation of customer interactions); ~30-50 rules from FTC's Sept 2023 + 2024 guidance documents. **Authoring path:** unlike `medical_pl` / `legal_pl` (gated on partner consent + Polish-legal-review owner), these 3 are from public regulatory text — no legal-review blocker for v1 placeholder authoring. Human-review pass at U16/U17 timing recommended but not blocking. **Default rule-set assignment:** `b2b_saas` / `b2b_ai_first` archetype = `gdpr_eu` (always) + `ai_disclosure_us` (always) + `ai_disclosure_eu` (when EU-targeting); `b2c_aesthetics` = `medical_*` + `gdpr_eu`; `b2b_regulated` = `legal_*` + `gdpr_eu`. **U16/U17 scope extended:** new units U16b (gdpr_eu authoring) + U16c (ai_disclosure_eu authoring) + U16d (ai_disclosure_us authoring) added in Phase D. **Plan impact:** §Scope Boundaries "Architecture supports but build deferred" entry for gdpr_eu + ai_disclosure_* REMOVED; rule sets now in v1 build. RUBRICS count assertion grows from 92 to 92 + (3 new compliance rule sets × 7 compliance-gated lanes per rule set, but per TD-18 v1 single-rule-set-per-client constraint means a client picks 1 active rule set, so RUBRICS count = 92 unchanged because rule-set rubric IDs are per-active-not-per-authored).

- **TD-47 / 2026-05-16 voice persona authoring workflow (hybrid LLM-extract + operator-curate).** Currently U3 specifies `VoicePersona` schema + loader but doesn't specify how a raw corpus becomes a curated persona. New workflow: `freddy voice extract --persona-ref <name> --corpus-path <path> [--locale <code>]`. **Pipeline:** load + chunk corpus → claude/opus prompted to extract pattern candidates (recurring sentence structures, vocabulary preferences, voice register, lived-work entities, phrasebook of high-frequency phrases) → emits `voice_persona_candidates.yaml` with proposed `voice_rules` + `style_anchors` + `phrasebook` → operator reviews/curates/commits as `voice_personas/<persona_ref>.yaml`. **For small corpora (<10K words, e.g., Klinika's `medycyna-urody`):** manual authoring stays the path; extractor is optional. **For large corpora (50-200+ posts, typical SaaS/AI client):** LLM-extraction is the high-leverage workflow; saves ~70% of operator time vs full manual extraction. **Multi-locale support:** `--locale` flag adapts the extractor prompt to evaluate patterns appropriate to the target language (Polish kreska/ogonek-aware tokenization, German compound-word splitting, etc.). **New module added to U3:** `src/voice/extractor.py` (~80 LOC) + `cli/freddy/commands/voice.py` extract subcommand (~40 LOC) + `tests/voice/test_extractor.py`. **Plan impact:** U3 Files list extended; voice_corpus authoring workflow documented in onboarding runbook (both regulated-vertical + SaaS/AI templates).

- **TD-48 / 2026-05-16 brief merging across source lanes — priority-weighted with per-client override.** TD-40 specified top-K=3 hybrid WITHIN a single source lane; this closes the cross-source-lane merge. **Default priority:** `marketing_audit` (highest authority — 149-lens human-reviewed) > `geo` (research-driven, structured findings) > `monitoring` (real-time signal, lower confidence) > `competitive` (lateral signal). **Tie-breaker:** per-brief `priority` field (high > medium > low) overrides source-lane order — a `high`-priority monitoring brief beats a `low`-priority marketing_audit brief. **Top-K=3 across the merged pool** (NOT per-source-lane K=3 each). **Per-client configurable:** `client.yaml.brief_consumption.source_lane_priority: [marketing_audit, geo, monitoring]` override. **Implementation:** `src/briefs/reader.py` extended with `read_briefs_merged(source_lanes: list[str], priority: list[str], top_k: int) -> list[FindingsBrief]`. **Used by:** article_engine (U13) primary consumer; site_engine (U15b) for marketing_audit+geo briefs; image_engine + ad_engine consume briefs for context but typically single-source per run. **Plan impact:** U4 Files list extended with `read_briefs_merged()`; U13 prose updated to reference merged read; new field on ClientConfig.

- **TD-49 / 2026-05-16 SaaS/AI client onboarding runbook template.** Currently U18 produces per-client launch runbooks (Klinika-specific, DWF-specific). For SaaS/AI primary v1.5+ mix, need a **template runbook** that's instantiated per-client. **New U18 deliverable:** `docs/runbooks/saas_ai_client_onboarding_template.md` — generic template alongside Klinika+DWF runbooks. **Differences from regulated-vertical runbook:**
   - **Voice corpus ingestion:** public-ingest path (blog archive + Twitter/LinkedIn data exports + customer interviews); near-zero consent friction; expected corpus size 50-200 posts; `freddy voice extract` CLI (per TD-47) is the default path
   - **Brand assets:** expect Figma exports / design system → Pydantic-validated `tokens.json` straight from designer; SVG logo + WOFF2 fonts both available; `brand_strictness: "strict"` works
   - **Compliance:** `gdpr_eu` rule set (always for EU clients) + `ai_disclosure_eu` (for AI-first companies in EU market) + `ai_disclosure_us` (always for US-targeting); `medical_*` / `legal_*` not needed
   - **GSC OAuth:** more likely already set up than for B2C clinics; verification flow same as TD-35
   - **Reviewer pattern:** product-marketing-manager OR founder primary; design-lead OR head-of-content secondary; **SLA pattern typically faster** (founder-approval-loops run in hours, not days) — default `sla_target: "24h_business"` vs Klinika's `"48h_business_pl"`
   - **Source material:** blog archive + product docs + customer interview transcripts → `clients/<slug>/source_material/`
   - **Site_engine `brand_strictness: "strict"`** default; minimal per-client rubric softening needed (gofreddy.ai SE-* anchors apply natively)
**Plan impact:** U18 Files list extended; template runbook authored as v1 deliverable (not per-client until first SaaS/AI client onboards).

- **TD-50 / 2026-05-16 cross-locale lane handling.** Locale moved to per-client config (TD-33), but lane-level non-English handling now specified.
   - **Per-locale rubric validation:** rubric anchors evaluate in client's locale; claude/opus + GPT-4 handle major European languages well (English, Polish, German, French, Spanish, Italian); less reliable for Japanese, Chinese, Arabic. **New flag `locale.requires_native_judge: bool`** (default `false` for European languages; `true` for Asian/Middle Eastern). When `true`, lane falls back to manual reviewer for rubric scoring until v1.5 vendor-side language support matures.
   - **Language-aware deny-regex:** `anti_patterns.yml` → `anti_patterns.<locale>.yml` (`anti_patterns.en.yml`, `anti_patterns.pl.yml`, etc.). v1 ships `.en.yml` (default, comprehensive) + `.pl.yml` (Klinika/DWF stub with Polish equivalents of major AI-tells); other locales added per first non-en/pl client.
   - **fal.ai prompts stay English** (Flux 2 + Imagen 4 best quality on English; copy in target language is separate). Voice corpus stays language-agnostic in pattern expression — `voice_rules` expressed as structural rules ("first-person register", "lived-work entities present"), not language-specific phrases.
   - **Compliance rule sets per language:** `medical_pl`, `medical_de`, etc. — authored on first client demand. `gdpr_eu` is language-agnostic (regulation applies regardless of content language).
**Plan impact:** U3 voice loader documents language-agnostic pattern expression; U5 compliance YAML schema allows multi-language rule sets; per-lane anti_patterns.yml restructured.

- **TD-51 / 2026-05-16 real_client fixture authoring at scale.** TD-38 specifies the strategy; this closes the operator-effort scaling question.
   - **v1 (Klinika + DWF launch):** JR + operator manually curate. ~20hr per onboarded client × 4 lanes; total ~40hr for v1 launch
   - **v1.5 trigger (client #4 onboarded):** hire freelance content editor (≤8hr/week, ~$50-100/hr; ~$1600-3200/mo) for fixture curation + cross-cycle calibration review. Editor doesn't need engineering background; needs content-quality judgment + familiarity with SaaS/AI marketing norms
   - **v2 trigger (client #8+):** in-house editor / content-ops role; editor curates fixtures + runs quarterly rubric calibration review + drafts client onboarding materials
   - **Per-archetype × lane fixture authoring effort:** ~3hr operator time. SaaS/AI archetype EASIER than regulated verticals (public corpora reduce curation time ~50% vs Klinika's private corpus pattern)
**Plan impact:** §Risks & Dependencies adds row on operator-time scaling; U19 success bar references the v1.5 trigger; budget envelope documented in §Operational Notes.

- **TD-52 / 2026-05-16 storyboard ↔ ad_engine: independent lanes in v1 (no handoff).** Per TD-45 video=scripts-only direction, ad_engine produces self-contained Reels Ad plan (no dependency on storyboard lane). Storyboard lane stays independent for storyboard-only outputs (educational videos, explainers — also script-only in v1). Soft dependency between U15 + U8 noted earlier in plan is REMOVED. **Rationale:** v1 ships both lanes as independent producers of video-plan artifacts; operator runs fal.ai I2V manually for both. v1.5 reconsiders unified video-orchestration if a client need surfaces (e.g., "produce 30 Reels + 5 explainers + 10 carousels per month, automated handoff between lanes saves operator time").
**Plan impact:** U15 ad_engine dependencies prose updated (remove "Soft dependency on U8 (storyboard produces ad Reels scripts)"); U8 storyboard scope unchanged.

- **TD-53 / 2026-05-16 overfitting reframes — 5 batch fixes.** Generic-first review of the plan flagged 5 places where Klinika+DWF specifics still read as design-center rather than first-onboarded-client examples. All applied:
   1. **SaaS/AI test fixtures added per lane.** New: `autoresearch/eval_suites/article_engine_saas_thought_leadership.json` (SaaS B2B blog fixture); `image_engine_saas_carousel.json` (LinkedIn document carousel for AI-infra company); `ad_engine_saas_linkedin.json` (LinkedIn Sponsored Content for B2B SaaS); `site_engine_gofreddy_hero.json` already exists (canonical SaaS/AI fixture). Klinika+DWF fixtures retained as regulated-vertical coverage.
   2. **§Risks voice corpus consent reframed.** Lead clause now: "Voice corpus consent: near-zero friction for SaaS/AI archetype with public content footprints (the expected v1.5+ majority); load-bearing for B2C-aesthetics like Klinika where corpus is private/proprietary." Klinika-specific posture retained as a per-client SHIP gate, not a default risk.
   3. **TD-12 per-client phasing reframed.** Now reads: "decision gate applies to any onboarded client whose compliance rule set isn't yet legal-reviewed AND whose corpus consent is load-bearing; SaaS/AI clients with `gdpr_eu`+`ai_disclosure_*` placeholder rule sets generally don't trigger this gate."
   4. **§Operational Notes locale lead.** Reordered to lead with SaaS/AI default (`en-US`, `gl=en/hl=en/google_domain=google.com`, `requires_diacritic_normalization=false`); Polish locale specifics moved to "Named per-client override examples" subsection.
   5. **Pre-publish reviewer SLA pattern.** Default SLA pattern now documented as `24h_business` for SaaS/AI archetype (product-marketing-manager OR founder reviewer); `48h_business_pl` for Klinika clinic-admin pattern; `72h_business` for DWF partner pattern. Per-client `sla_pattern` field in `ClientConfig.pre_publish_reviewer.sla_target` selects default by archetype.
...
| Archetype | Description | Default compliance rule sets | Typical brand-asset maturity | Typical voice-corpus shape | v1.5+ priority |
|---|---|---|---|---|---|
| `b2b_saas` | SaaS tools, dev tools, vertical SaaS, product-led-growth | `gdpr_eu` + `ai_disclosure_us` (`+ ai_disclosure_eu` if EU-targeting) | High (Figma design system, SVG logos, defined typography) | Public blog archives (50–200 posts), founder Twitter/LinkedIn, customer interviews | Primary |
| `b2b_ai_first` | LLM apps, AI infrastructure, AI agencies, model API consumers | `gdpr_eu` + `ai_disclosure_eu` + `ai_disclosure_us` | High (design-system-driven) | Public docs + founder content + customer interviews | Primary |
| `b2c_ecommerce` | DTC brands, online stores, marketplaces, vertical e-com | `ecom_consumer_us` + `gdpr_eu` (`+ ftc_endorsement_us` if influencer collabs) | Medium-High (brand book + product photography library, sometimes design system) | Product descriptions + customer reviews + brand-story content | Primary |
| `creator_solo` | Individual creator/influencer with owned audience (newsletter, podcast, YouTube, course creator) | `ftc_endorsement_us` + `gdpr_eu` | Low-Medium (personal brand: logo + signature colors; rarely a system) | Past content + tweets + video transcripts + audience comments + newsletter archives | Primary |
| `creator_managed` | Creator with team/manager/agency (same content shape, different reviewer flow) | `ftc_endorsement_us` + `gdpr_eu` | Same as solo (often slightly more mature) | Same as solo | Primary |
| `b2b_agency` | Marketing/design/dev agencies selling to other businesses; consultancies | `gdpr_eu` (`+ ai_disclosure_us` if AI-services agency) | Medium (agency brand book; some Figma) | Case studies + client work + thought leadership + partner content | Secondary |
| `indie_hacker` | Solo founder running small SaaS / side projects; usually founder-authored content | `gdpr_eu` (minimal compliance posture) | Low (logo + minimal brand) | Twitter + dev blog + Product Hunt + Indie Hackers posts | Secondary |
| `b2c_aesthetics` | Aesthetic / wellness / dental / dermatology (Klinika family) — **regulated B2C, first onboarded example only** | `medical_*` per-country + `gdpr_eu` (strict) | Low-Medium (often only logo + photography library) | Often private (consent-required); fallback to `style_anchors`-only is degraded | First-onboarded example; not primary growth target |
| `b2b_regulated` | Law, finance, healthtech, fintech, public-sector consultancies (DWF family) — **regulated B2B, first onboarded example only** | `legal_*`/`fintech_*`/`healthtech_*` + `gdpr_eu` (strict) | Medium (logos + brand book; rarely a design system) | Mixed (some public partner content; mostly private) | First-onboarded example; not primary growth target |
| `b2b_consultancy` | Advisory/consultancy firms (non-legal, non-medical) | `gdpr_eu` (`+ ai_disclosure_us` for AI-advisory) | Medium | Whitepapers + insights + case studies | Secondary |
...
**v1 compliance rule sets (BUILD in v1 per TD-46 + TD-54):**
- `gdpr_eu` — baseline for any EU-targeting client (most internet clients of any archetype)
- `ai_disclosure_eu` — EU AI Act Art. 50 transparency (AI-first companies in EU market)
- `ai_disclosure_us` — FTC AI advertising guidance (default for any AI-claiming client US-targeting)
- `ftc_endorsement_us` (**NEW per TD-54**) — FTC creator endorsement disclosure rules (load-bearing for `creator_*` archetypes; also affects `b2c_ecommerce` doing influencer collabs)
- `ecom_consumer_us` (**NEW per TD-54**) — FTC consumer advertising substantiation (load-bearing for `b2c_ecommerce` archetype; product-claim backing, comparison-claim integrity, "free trial" disclosure rules)
...
**Architecture supports v1.5 candidates** (authored on first prospect): `medical_de`/`medical_fr` (per-country medical), `legal_*` per-jurisdiction (UK SRA, US bar rules), `soc2_b2b`, `fintech_*`, `healthtech_*`, `dsa_eu` (EU Digital Services Act for platforms), `coppa` (US under-13 protection), `tcpa` (US text-message marketing), `casl` (Canadian anti-spam).
...
- N compliance rule sets (architecture takes any; **v1 ships 5 rule sets** per TD-46: `medical_pl` for Klinika, `legal_pl` for DWF, **plus `gdpr_eu` + `ai_disclosure_eu` + `ai_disclosure_us`** authored from public regulatory text — no legal-review-owner blocker for these 3, ready for first SaaS/AI onboarding). **v1.5 candidates** (authored on-demand at first prospect of each archetype): `soc2_b2b`, `fintech_*`, `healthtech_*`, `medical_de` / `medical_fr` / per-EU-country medical, `legal_*` per-jurisdiction. v1 ships with placeholder rule sets in U16/U17 for `medical_pl`/`legal_pl` until legal review completes; new units U16b/U16c/U16d (per TD-46) author `gdpr_eu` + `ai_disclosure_eu` + `ai_disclosure_us` in v1 build. **Default rule-set assignment per archetype:** `b2b_saas`/`b2b_ai_first` = `gdpr_eu` + `ai_disclosure_us` + `ai_disclosure_eu` (EU-targeting); `b2c_aesthetics` = `medical_*` + `gdpr_eu`; `b2b_regulated` = `legal_*` + `gdpr_eu`.
...
| 5 | `src/evaluation/rubrics.py` | Register per-lane rubric prose for `<LANE_PREFIX>-1..8` (AE/IE/AD inline; SE via `RubricTemplate.prose_ref → docs/rubrics/site-quality.md` per TD-30). **`RubricTemplate.prose_ref` field does NOT yet exist on the dataclass** (current fields: `criterion_id`, `domain`, `scoring_type`, `prompt`, `is_cross_item`, `tier` — verified 2026-05-15 substrate audit on `src/evaluation/rubrics.py:16-27`). U5 owns the substrate addition (`prose_ref: str | None = None` + resolver helper that reads `compliance/rule_sets/<name>.yaml` AND file-anchor refs like `docs/rubrics/site-quality.md#se-N`); U13/U14/U15 add inline AE/IE/AD prose without using `prose_ref`; U15b consumes the U5-added field. **Compliance rubric IDs per D12-hybrid:** the lane's `LaneSpec.rubric_ids` carries the per-lane compliance ID (e.g., `medical_pl_article_engine_compliance` — one entry per active rule set per lane, NOT one per individual rule). That ID exists in `RUBRICS` as a `RubricTemplate` whose prose is a resolve-at-eval-time reference to `compliance/rule_sets/<rule_set>.yaml` via `prose_ref`. The derived `len(RUBRICS) == sum(len(spec.rubric_ids) for spec in LANES.values())` assertion (D4) covers both per-lane + compliance IDs automatically — no separate count. **`render_rubric_ids` (RND-1..5) are NOT in `RUBRICS`** — they're parallel rubrics owned by `render_judge.py` scripts per-archive (inline `CRITERIA = ["RND-1", ..., "RND-5"]`) and so are NOT counted against the derived sum. Final v1 count = 53 baseline + 32 new-lane (4 × 8) + 7 compliance IDs (one per compliance-gated lane × single-rule-set-per-client D6/TD-18) = **92 total**. | RUBRICS assertion fails on lane load; or — if `prose_ref` not added in U5 — U15b's SE-* anchor resolution raises `AttributeError` at first eval. |
...
- **Archetype enum + stub_allowed flag (expanded per TD-33 + TD-54 generic-first generalization, 11 archetypes):** `ClientConfig.archetype: Literal["b2b_saas", "b2b_ai_first", "b2c_ecommerce", "creator_solo", "creator_managed", "b2b_agency", "b2b_consultancy", "indie_hacker", "b2c_aesthetics", "b2b_regulated", "b2b_tech"]` (Literal extended as new archetypes are added). **Primary expected client mix:** `b2b_saas`, `b2b_ai_first`, `b2c_ecommerce`, `creator_solo`, `creator_managed`. **Secondary:** `b2b_agency`, `b2b_consultancy`, `indie_hacker`. **First-onboarded examples only (not design targets):** `b2c_aesthetics` (Klinika), `b2b_regulated` (DWF). **Stub:** `b2b_tech` (U19 architectural validation). Optional `archetype_stub_allowed: bool = False` excludes the archetype from D11's CI assertion (≥1 real_client fixture per archetype). v1 sets `stub_allowed=True` only for `b2b_tech` in `_stub_b2b_tech/client.yaml` (U19); all other archetypes default `stub_allowed=False`. Per-archetype defaults (voice corpus shape, brand asset maturity, compliance rule sets, reviewer SLA, source material) documented in §Operational Notes archetype-defaults reference table per TD-54.
...
- **NEW `ClientConfig.voice_corpus_consent_required` field (per TD-33):** `voice_corpus_consent_required: bool` — defaults derived from archetype: `b2b_saas` / `b2b_ai_first` / `b2b_consultancy` default `false` (public content corpus, no consent friction); `b2c_aesthetics` defaults `true` (Klinika's medycyna-urody corpus is private/proprietary); `b2b_regulated` defaults `true` (partner content). Parallel-track risk #1 reframed: consent is a per-client property, not a default risk.
...
- **Create: `src/voice/extractor.py` (~80 LOC per TD-47)** — hybrid LLM-extract + operator-curate workflow. `extract_persona_candidates(corpus_path: Path, locale: str) -> dict` chunks corpus → claude/opus extracts pattern candidates (recurring sentence structures, vocabulary preferences, voice register, lived-work entities, phrasebook of high-frequency phrases) → emits `voice_persona_candidates.yaml` with proposed `voice_rules` + `style_anchors` + `phrasebook`. Operator reviews/curates/commits as `voice_personas/<persona_ref>.yaml`. Multi-locale aware via `locale` param (Polish kreska/ogonek-aware tokenization, German compound-word splitting, etc.).
- **Create: `cli/freddy/commands/voice.py` (~40 LOC per TD-47)** — `freddy voice extract --persona-ref <name> --corpus-path <path> [--locale <code>]` subcommand. Runs `extract_persona_candidates()`, writes `voice_persona_candidates.yaml` for operator review.
- **Create: `tests/voice/test_extractor.py`** — unit tests for extractor pipeline.
...
- Create: `src/briefs/reader.py` — `read_briefs(source_lane: str, archive_root: Path) -> list[FindingsBrief]` PLUS **`read_briefs_merged(source_lanes: list[str], priority_order: list[str], top_k: int, archive_root: Path) -> list[FindingsBrief]` (per TD-48)**. Merges across multiple source lanes with priority-weighted ordering (default `marketing_audit > geo > monitoring > competitive`); per-brief `priority` field (high > medium > low) overrides source-lane order; top-K applied across the merged pool, not per-source. Per-client configurable via `ClientConfig.brief_consumption.source_lane_priority`.
...
- **Create: `compliance/rule_sets/gdpr_eu.yaml` (per TD-46)** — baseline rule set for EU-targeting clients (mandatory for `b2b_saas`/`b2b_ai_first` EU clients). ~80-150 rules from public GDPR articles: data-handling claims, cookie disclosures, retention representations, right-to-erasure, lawful basis representations, data subject rights, cross-border transfers (post-Schrems II). Authored in U16b (new unit Phase D); no legal-review blocker since public text.
- **Create: `compliance/rule_sets/ai_disclosure_eu.yaml` (per TD-46)** — EU AI Act Art. 50 transparency obligations. ~30-50 rules: AI-generated content marking, deepfake labeling, emotional-recognition system disclosure, AI system interaction disclosure. Authored in U16c (new unit Phase D).
- **Create: `compliance/rule_sets/ai_disclosure_us.yaml` (per TD-46)** — FTC AI advertising guidance (Sept 2023 + 2024 docs). ~30-50 rules: no false AI capability claims, no undisclosed customer-interaction automation, AI training-data representations, substantiation requirements for AI performance claims. Authored in U16d (new unit Phase D).
- **Create: `compliance/rule_sets/ftc_endorsement_us.yaml` (per TD-54)** — FTC Endorsement Guides (2023 revision) + Native Advertising Guides. ~40-60 rules: material connection disclosure ("#ad" / "Paid partnership with [brand]"), placement and conspicuousness rules, platform-specific disclosure conventions (Instagram/TikTok/YouTube/podcast/newsletter), child-endorser restrictions, AI-generated endorsement disclosure (2024 FTC update). **Load-bearing for `creator_solo`, `creator_managed`, and `b2c_ecommerce` archetypes** (the latter when running influencer campaigns or affiliate programs). Authored in U16e (new unit Phase D).
- **Create: `compliance/rule_sets/ecom_consumer_us.yaml` (per TD-54)** — FTC consumer advertising substantiation + state-level e-com rules. ~50-80 rules: product-claim substantiation requirements (Section 5 FTC Act), comparison-claim integrity (substantiation + apples-to-apples), free-trial offer disclosure rules (Mail/Telephone Order Rule + 30-Day Rule), "Made in USA" qualification, environmental marketing claims (Green Guides), recurring-billing disclosure (ROSCA), online review authenticity (FTC Aug 2024 rule on fake reviews), pricing claim integrity ("from $X" requires meaningful inventory at that price). **Load-bearing for `b2c_ecommerce` archetype**. Authored in U16f (new unit Phase D).
...
- Create: `src/evaluation/rubric_resolver.py` (~40 LOC) — `resolve_prose(template: RubricTemplate, registry_root: Path) -> str`. Dispatches on `prose_ref` shape: `compliance/rule_sets/<name>.yaml#<rule_id>` → loads YAML, returns rule prose; `docs/rubrics/<file>.md#<anchor>` → loads markdown section by `## <anchor>` heading; falls back to `template.prompt` when `prose_ref is None`. Cached per-process via `functools.lru_cache` keyed on `(prose_ref, file mtime)`. Consumed by `evaluate_variant.py` at judge-dispatch time AND by `src/compliance/judge.py:evaluate_compliance`.
- Modify: `src/evaluation/rubrics.py` — register per-lane compliance rubric duplicates as `RubricTemplate(criterion_id=..., domain=..., scoring_type="gradient", prompt="", prose_ref="compliance/rule_sets/<rule_set>.yaml#<lane>_compliance", ...)`.
...
- Test: `tests/evaluation/test_rubric_resolver.py` — verifies prose_ref resolution for both YAML rule-set refs and markdown anchor refs; verifies fallback to inline `prompt` when `prose_ref is None`; verifies cache invalidates on file mtime change.
...
- [ ] **U6b: Moment-emission helper (`emit_moment`) — shared-infra primitive (portal coordination)**

**Goal:** A thin canonical-event helper that every new lane + U7 calls at meaningful checkpoints, so the client portal's moments timeline surfaces what each lane is doing for a client. Coordination point with the portal-moments redesign at `docs/brainstorms/2026-05-15-portal-moments-redesign-requirements.md` (R-Lane-1, R-Lane-2).

**Requirements:** portal-redesign R-Lane-1 (helper API), R-Lane-2 (minimum emission kinds), R-Schema-1..3 (`KNOWN_KINDS` + `CANONICAL_FIELDS` extensions).

**Dependencies:** none new (uses existing `autoresearch.events.log_event` + `client_events_path` from PR #61 P1).

**Files:**
- Modify: `autoresearch/events.py` — add `emit_moment(client_id, moment_kind, title, *, source_event_ids=None, body=None, **metadata)` wrapper that sets `kind="moment"` + canonical fields and emits via `log_event(...)` to `client_events_path(client_id)`. ~30 LOC.
...
- Modify: `tests/autoresearch/test_events.py` — drift tests cover new kinds + canonical fields; add `test_emit_moment_smoke`, `test_emit_moment_title_truncation`, `test_emit_moment_routes_per_client`.

**Approach:**
Helper signature: `emit_moment(client_id: str, moment_kind: str, title: str, *, source_event_ids: list[str] | None = None, body: str | None = None, **metadata) -> None`. Sets `kind="moment"`, fills `metadata.moment_kind = moment_kind`, `metadata.title = title`, optional `metadata.body`, optional `metadata.source_event_ids` array. Routes via `client_events_path(client_id)`. Title ≤120 chars; longer titles truncated with `…` and logged at WARN. The helper is a thin pass-through; no buffering, no transformation beyond field-shape enforcement.

**Minimum moment kinds every new lane MUST emit (R-Lane-2):**
...
- **U7** emits `review_required` (when handing off to a reviewer) via the existing `events.log_event(kind="review_required", ...)` path now that the kind is in `KNOWN_KINDS` (top-level kind, not a moment subtype). U7's `review_approve`/`review_reject`/`sla_breach` continue via `events.log_event(kind=...)` per the Edit-1 changes below.
...
**Patterns to follow:**
- `src/common/cost_recorder.py` — already mirrors `kind="cost"` to canonical log via `log_event`. The `emit_moment` helper follows the same pattern of "thin canonical-event emission with sensible defaults."

**Test scenarios:**
- *Happy path:* `emit_moment("klinika-melitus", "session_start", "Marketing audit started")` writes a canonical event with `kind="moment"`, `metadata.moment_kind="session_start"`, `metadata.title="Marketing audit started"`, path resolves to `clients/klinika-melitus/audit/events.jsonl`.
- *Truncation:* title >120 chars → truncated to 119 + `…`, WARN logged once per call.
- *Citation:* `source_event_ids=["evt_1", "evt_2"]` preserved in `metadata.source_event_ids`.
...
- All new content lanes (U13/U14/U15/U15b) + modified lanes (U8 storyboard, U10b marketing_audit brief) + U7 call `emit_moment` at the required checkpoints (verified per the New-lane Substrate Wiring Checklist item added in this pass, plus a cross-lane integration test that runs each lane against a fixture and asserts at least one `session_start` + one `deliverable_ready` + one `session_completed` moment lands in `clients/<slug>/audit/events.jsonl`).

**Reversibility:** Fully reversible. The helper is purely additive; existing lanes ignore it; removing it un-blocks only the portal's moments timeline (the portal degrades to system-emit-only display).
...
**Cross-cutting requirement for every Phase B unit (added 2026-05-18 portal coordination):** all lanes modified or added in this phase MUST call `events.emit_moment(client_id, moment_kind, title, ...)` per the U6b contract at minimum the three checkpoints in R-Lane-2: `session_start`, `deliverable_ready`, `session_completed`. This is verified by the cross-lane integration test in U6b. Optional checkpoints (`decision_logged`, `error_recovered`) are lane-author discretion.
...
- **Create: `autoresearch/eval_suites/article_engine_saas_thought_leadership.json` (per TD-53 #1)** — SaaS B2B thought-leadership fixture (`b2b_saas` archetype coverage). Source: gofreddy.ai blog archive or analogous AI-infra/dev-tools content (Linear/Vercel/Replicate blog style).
- **Create: `autoresearch/eval_suites/article_engine_ecom_brand_story.json` (per TD-54)** — DTC e-commerce brand-story / product-launch article (`b2c_ecommerce` archetype coverage). Synthetic at v1; becomes real_client when first e-com client onboards.
- **Create: `autoresearch/eval_suites/article_engine_creator_newsletter.json` (per TD-54)** — Creator/influencer newsletter-essay fixture (`creator_solo` archetype coverage). Synthetic at v1; becomes real_client when first creator client onboards. Source pattern: Lenny Rachitsky / Stratechery / Lex Fridman blog / Subtack-leading-writers style.
- **Create: `autoresearch/eval_suites/article_engine_agency_case_study.json` (per TD-54)** — B2B agency case-study fixture (`b2b_agency` archetype coverage). Synthetic at v1.
...
- **Create: `autoresearch/eval_suites/image_engine_saas_carousel.json` (per TD-53 #1)** — LinkedIn document carousel for AI-infra/dev-tools company (`b2b_saas`/`b2b_ai_first` archetype coverage).
- **Create: `autoresearch/eval_suites/image_engine_ecom_ig_carousel.json` (per TD-54)** — DTC product-launch IG carousel (`b2c_ecommerce` archetype coverage). Synthetic at v1.
- **Create: `autoresearch/eval_suites/image_engine_creator_ig_single.json` (per TD-54)** — Creator/influencer IG single image with personal-brand stamp (`creator_solo` archetype coverage). Synthetic at v1.
- **Create: `autoresearch/eval_suites/image_engine_agency_doc_carousel.json` (per TD-54)** — B2B agency case-study LinkedIn document carousel (`b2b_agency` archetype coverage). Synthetic at v1.
...
- **Create: `autoresearch/eval_suites/ad_engine_saas_linkedin.json` (per TD-53 #1)** — LinkedIn Sponsored Content + Document Ad for B2B SaaS (`b2b_saas`/`b2b_ai_first` archetype coverage).
- **Create: `autoresearch/eval_suites/ad_engine_ecom_meta.json` (per TD-54)** — DTC product Meta Image Ad + Reels Ad plan (`b2c_ecommerce` archetype coverage). Synthetic at v1. Tests `ecom_consumer_us` substantiation gates + free-trial disclosure rules.
- **Create: `autoresearch/eval_suites/ad_engine_creator_meta.json` (per TD-54)** — Creator-led Meta sponsored post / sponsored Reel (`creator_solo`/`creator_managed` archetype coverage). Synthetic at v1. Tests `ftc_endorsement_us` material-connection disclosure gates ("#ad", "Paid partnership with [brand]" placement + conspicuousness).
...
- Compliance gate: per-client `compliance_rule_sets` (Klinika `medical_pl` softens Meta health-vertical word avoidance; DWF `legal_pl`; SaaS clients default `gdpr_eu` + optional `ai_disclosure_eu` / `ai_disclosure_us`).
...
- **Create: `autoresearch/eval_suites/site_engine_ecom_hero.json` (per TD-54)** — DTC e-commerce hero-section fixture (`b2c_ecommerce` archetype coverage). Synthetic at v1.
- **Create: `autoresearch/eval_suites/site_engine_creator_about.json` (per TD-54)** — Creator/influencer About + Newsletter-signup section (`creator_solo` archetype coverage). Synthetic at v1.
...
- **Create: `docs/runbooks/saas_ai_client_onboarding_template.md` (per TD-49)** — generic onboarding runbook template for the expected v1.5+ SaaS/AI primary client mix. Sections: (1) GSC OAuth setup (TD-35 shared OAuth client + per-client refresh token; typically pre-existing for SaaS clients); (2) Voice corpus public-ingest via `freddy voice extract --persona-ref <name> --corpus-path <path>` (per TD-47) against blog archive + LinkedIn/Twitter exports + customer interview transcripts (no consent friction); (3) Brand asset gathering from client's Figma design system (SVG logo + WOFF2 fonts + design tokens → `tokens.json` straight from designer; `brand_strictness: "strict"` default); (4) Compliance rule set assignment (`gdpr_eu` + `ai_disclosure_us` always; `ai_disclosure_eu` for EU-targeting; per TD-46); (5) Reviewer pattern (product-marketing-manager OR founder primary; design-lead OR head-of-content secondary; default SLA `24h_business` — faster than Klinika's `48h_business_pl`); (6) Source material gathering (blog archive + product docs + customer interview transcripts → `clients/<slug>/source_material/`); (7) Site_engine site URL + sections_in_scope (typically full set hero/value_prop/social_proof/faq/cta/pricing — SaaS sites have all 6); (8) First-launch demo pipeline (run 1 article + 1 ad + 1 site section variant; reviewer signs off; first published artifact within 7 days). Template instantiated per-client at SaaS/AI client onboarding time.
...
| AI-generated content fails compliance review (silent fallout) | Per-client `content_denylist` enforces vertical-specific exclusions at variant generation. **Examples across archetypes:** SaaS/AI clients deny `customer_pii` + `unreleased_feature_references`; e-com clients deny `unsubstantiated_health_claims` + `fake_metric_callouts`; creators deny `undisclosed_paid_partnerships` (covered by `ftc_endorsement_us`) + `audience_overlap_misrepresentation`; agencies deny `client_confidential_methodology`; first-onboarded `b2c_aesthetics` (Klinika) denies `clinical_visuals` + `before_after_imagery`. v1 compliance gate stack (per TD-46 + TD-54): `gdpr_eu` (baseline) + `ai_disclosure_eu`/`ai_disclosure_us` (AI clients) + `ftc_endorsement_us` (creator/affiliate) + `ecom_consumer_us` (e-com) + `medical_*`/`legal_*` (first-onboarded regulated examples). Pre-publish human review (U7) is the safety net beyond compliance gate. |
...
|---|---|---|---|---|---|---|---|
| `b2b_saas` | Public blog (50-200 posts) + LinkedIn + customer interviews | Figma design system → tokens.json | `gdpr_eu` + `ai_disclosure_us` (`+ ai_disclosure_eu` if EU) | Product-marketing-manager primary; design-lead secondary | `24h_business` | Blog archive + product docs + customer transcripts | `strict` |
| `b2b_ai_first` | Public docs + founder LinkedIn + customer interviews | Figma design system | `gdpr_eu` + `ai_disclosure_eu` + `ai_disclosure_us` | Founder primary; PMM secondary | `24h_business` | Docs + research notes + customer transcripts | `strict` |
| `b2c_ecommerce` | Product descriptions + customer reviews + brand-story content | Brand book + product photography library | `ecom_consumer_us` + `gdpr_eu` (`+ ftc_endorsement_us` if running influencer campaigns) | Marketing manager primary; founder secondary | `24h_business` | Product catalog + customer reviews + brand story | `strict` (Figma-mature DTCs) OR `permissive` (early-stage DTCs) |
| `creator_solo` | Past content (posts/videos/podcasts) + tweets + transcripts + newsletter archives + audience comments | Personal brand (logo + signature colors) | `ftc_endorsement_us` + `gdpr_eu` | Themselves (no secondary) | `12h_business` (fast iteration) | Content archive + audience analytics | `permissive` (personal brand, often non-system) |
| `creator_managed` | Same as solo (often slightly more curated) | Same as solo (sometimes more mature) | `ftc_endorsement_us` + `gdpr_eu` | Creator primary; manager secondary | `24h_business` | Same as solo | `permissive` or `strict` |
| `b2b_agency` | Case studies + thought leadership + client work + partner content | Agency brand book | `gdpr_eu` (`+ ai_disclosure_us` if AI services) | Senior partner primary; marketing lead secondary | `48h_business` | Case studies + client testimonials + whitepapers | `strict` |
| `b2b_consultancy` | Whitepapers + insights + case studies | Brand book | `gdpr_eu` (`+ ai_disclosure_us` for AI-advisory) | Senior consultant primary; managing partner secondary | `48h_business` | Whitepapers + research + frameworks | `strict` |
| `indie_hacker` | Twitter + dev blog + Product Hunt + Indie Hackers posts | Logo + minimal brand (often no design system) | `gdpr_eu` (minimal posture) | Themselves (no secondary) | `12h_business` (fast iteration) | Blog + Twitter archive + product changelog | `permissive` |
| `b2c_aesthetics` (first-onboarded — Klinika) | Private corpus (consent-required); fallback degraded | Photography library + style guide | `medical_*` per-country + `gdpr_eu` | Clinic owner/doctor primary; clinic manager secondary | `48h_business_pl` | Procedure docs + patient education materials | `permissive` |
| `b2b_regulated` (first-onboarded — DWF) | Partner articles + research notes | Brand book | `legal_*`/`fintech_*`/`healthtech_*` + `gdpr_eu` | Partner primary; senior associate secondary | `72h_business` | Partner publications + regulatory primers + case-law summaries | `strict` |
