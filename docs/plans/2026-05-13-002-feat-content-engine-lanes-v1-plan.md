...
| D27 | site_engine **rubric prose lives in a top-level dedicated file** at `docs/rubrics/site-quality.md` (frontmatter `version: v1`, content: SE-1..SE-8 with operational-definition + falsifiability + Score-3-failure-modes + anti-gaming clauses per S2 anchor-design pattern). `RubricTemplate.prose_ref` in `rubrics.py` resolves to this file at evaluation time. | Substantial rubric prose (~350 lines) earns top-level `docs/rubrics/` placement and independent versioning over inline per-plan anchor files. Adds zero substrate complexity: `prose_ref` is the same resolution mechanism `compliance_rules` uses per TD-11. Per-plan anchor convention (`docs/plans/<plan-id>-<LANE>-rubric-anchors.md`) stays for AE/IE/AD which are smaller and lane-scoped. |
