...
- Fixture snapshot capture is an operator one-liner via Playwright (`npx playwright screenshot --full-page <url> hero-desktop.png` + manual HTML/CSS save). Documented in §Operational Notes; not a tracked Python module.
- Existing Bleach/nh3 test suites cover XSS filter-evasion at the library level — no separate OWASP corpus test maintained here. `test_site_engine_substrate.py` includes 3-4 representative round-trip assertions (`<script>`, `<iframe srcdoc>`, `javascript:` URL) to verify the sanitizer is wired correctly; vector exhaustiveness is the library's responsibility.
