...
| D2 | Mirror x_engine/linkedin_engine pattern for `article_engine`, `image_engine`, `ad_engine`, **`site_engine`** — including `count_findings → 0` and `findings_promotion` for substrate compatibility. site_engine adds a structural pre-render gate (U7b render + U7c audit) BEFORE LLM scoring; all other content-lane conventions hold. | x_engine port master plan v13 + linkedin_engine v040 cold-start fix established the content-lane pattern. site_engine's pre-render gate is the same shape as x_engine's slop-check subprocess — additive substrate, no invariant change. |
| D3 | New rubric ID prefixes: **AE** (article_engine), **IE** (image_engine), **AD** (ad_engine), **SE** (site_engine). 8 rubrics each for the four new lanes (per `_rubric_ids` helper); 6 each for x_engine/linkedin_engine stay unchanged. SE-1..SE-8 prose lives in `docs/rubrics/site-quality.md` per TD-30 (dual-consumer file); AE/IE/AD prose lives inline in `rubrics.py` per the original convention. | Avoid prefix collisions; 8-rubric default matches GEO/CMP/MON/SB/MA. Content lanes' rubric prose authoring is the long pole (~30–40 lines × 32 rubrics ≈ 960–1280 lines; site_engine's SE-1..SE-8 already authored in `docs/rubrics/site-quality.md` — saves ~250 lines). |
...
| D25 | Compliance-judge backend (used by U5's `evaluate_compliance` for the LLM-judged portion of the compliance rubric) = **claude/opus**. Held in a singleton `ComplianceJudgeConfig` module owned by U5 (`src/compliance/judge_config.py`, ~15 LOC). Applies to the **7 lanes** that carry compliance gates in v1 (storyboard, article_engine, image_engine, ad_engine, **site_engine**, linkedin_engine, x_engine). Geo has no compliance gate in this plan. | Compliance correctness is the most-consequential judge call (false-negative = regulator-flagged published artifact). Frontier-class + diverse from inner-loop (codex/gpt-5.5) per `judge-decisions-2026-05-11.md`. Single shared backend prevents per-lane drift in Art. 14 / legal_pl interpretation. **Why U5-owned singleton instead of a per-lane `LaneSpec` field:** a 7-times-duplicated `LaneSpec.compliance_judge_backend` field would drift; U5 is already the single point of compliance invocation, so the singleton lives there. Operator override available via `COMPLIANCE_JUDGE_BACKEND` / `COMPLIANCE_JUDGE_MODEL` env vars (resolution mirrors `cc212c2` per-step-split conventions). |
| D26 | site_engine **vision-sub-judge dispatch** mirrors D24 image_engine pattern. Visual rubrics SE-1 (hierarchy), SE-5 (brand-token fit), SE-8 (anti-slop) dispatch to **Gemini 2.5** via `src/evaluation/vision_judge.py` (the D24-introduced rubric-driven judge primitive). Text rubrics SE-2 (copy clarity), SE-3 (claim honesty), SE-4 (voice fit) dispatch to **claude/opus** (text outer judge). Structural rubrics SE-6 (a11y), SE-7 (perf) are **not LLM-scored** — computed directly from U7c `A11yReport` and `PerfReport` per `docs/rubrics/site-quality.md` falsifiability rules. | Same multimodal split as image_engine; visual axes can't be scored text-only; structural axes are deterministic. Reuses the D24 vision-judge primitive without growing vendor surface — Gemini 2.5 is already wired for `image_preview_service.py` + image_engine's vision sub-judge. Verified by `test_site_engine_substrate.py` per-axis dispatch test (U15b). |
| D27 | site_engine **rubric prose lives in a single file consumed by two surfaces** (the lane + the `compound-engineering:site-improvement` Claude Code skill). File: `docs/rubrics/site-quality.md` (frontmatter `version: v1`, content: SE-1..SE-8 with operational-definition + falsifiability + Score-3-failure-modes + anti-gaming clauses per S2 anchor-design pattern). `RubricTemplate.prose_ref` in `rubrics.py` resolves to this file at evaluation time. | Skill + lane share the same opinions — edit once, propagate both. Per-plan anchor convention (`docs/plans/<plan-id>-<LANE>-rubric-anchors.md`) reserved for lane-only consumers (AE/IE/AD); the new `docs/rubrics/` location is justified by dual consumption. Adds zero substrate complexity: `prose_ref` is the same resolution mechanism `compliance_rules` uses per TD-11. |
...
Phase A — Foundations
  U1 (RUBRICS derived count)
    ...
    │     │     │
    │     │     └─→ U7c (a11y + perf audit utility — shared)             ← needs U7b
    │     │            │
    ...
    └─→ U15b (site_engine lane)       ← needs U2, U3, U4, U5, U7, U7b, U7c
    │
...
site_engine:                        # per TD-28: section-level v1 scope
  ...
  perf_budget_override: null        # uses defaults from src/generation/site_audit.py:PERF_BUDGETS
...
- **Paused-N-days operator override backstop (per Pass-4 reliability audit):** if an artifact has been auto-paused for ≥ `OPERATOR_OVERRIDE_DAYS` (default 7) AND no reviewer (primary or secondary) has clicked, operator can override-publish via `freddy autoresearch review override --artifact-id <id> --reason "<text>"` CLI subcommand. Override is logged with `decision: operator_override`, `decision_timestamp`, `reason_text`, and reviewer-email-state at override time. Used when both reviewers genuinely unavailable (vacation, departure, illness). NOT a substitute for review — alert operator at 5-day pause threshold so they reach out before override is needed. Without this backstop, both-reviewers-unavailable cascades into indefinite queue + cold-start regression on the lane (per linkedin_engine v040 lesson).
...
- *Integration:* Pause persists ≥ `OPERATOR_OVERRIDE_DAYS` (default 7) → operator CLI `freddy autoresearch review override` publishes with reason logged; subsequent re-review path tested.
...
- **Cyber-filter session-marker observability (per Pass-4 reliability audit):** because codex rejections produce session markers (e.g., `"Session BLOCKED: terminal agent marker"` per memory: `project-geo-regression-root-cause-2026-05-12.md`) rather than raised exceptions, U13 wires a session-marker scrape into the metrics path: every variant's run captures `session_marker_count` and `session_marker_reasons` from the inner-session transcript. Both fields land in `autoresearch/metrics/generations.jsonl` per existing convention. A daily aggregation step (cron) emits an operator alert when any lane's `session_marker_count / variant_count` ratio exceeds 5% over a 24h window — this catches silent rejection cascades at scale (1% × thousands of variants over 3-5mo build = real degradation that looks like "low-scoring variants" to the substrate). Applies to U13 + U14 + U15 + U15b identically.
...
- Create: `autoresearch/eval_suites/site_engine_gofreddy_hero.json` — gofreddy.ai hero-section fixture (canonical training example from 2026-05-13 session). **Snapshot-pinned, not re-fetched:** the fixture bundle includes captured `hero.html` + `hero.css` + `hero-desktop.png` + `hero-mobile.png` + `dom-snapshot.txt` checked into the repo at `autoresearch/eval_suites/fixtures/site_engine_gofreddy_hero/`. The fixture JSON references these by path, NOT by URL. Prevents fixture drift if gofreddy.ai is edited or unreachable during the 3-5 month build window. Refresh procedure: capture-once via `scripts/snapshot_site_fixture.py <url> <fixture_name>` on a clean run, commit, never re-fetch live. See parallel-track risk #11.
- Create: `scripts/snapshot_site_fixture.py` (~80 LOC) — captures HTML/CSS/screenshot/DOM-snapshot from a URL into a fixture directory. Used once per fixture creation. Operator-only tool, not part of the runtime path.
- Create: `tests/autoresearch/test_site_engine_sanitizer.py` (~80 LOC) — fed OWASP XSS filter-evasion corpus as input; asserts every vector either fails to round-trip through the sanitizer OR strips to an inert form. Updated when new bypass classes surface (annual review cadence).
...
- **Structural gate (two-pass)** runs BEFORE LLM scoring:
  ...
  - **Pass 2 — render + audit.** U7b render then U7c audit. Fail if: render fails (Chromium can't load section); `console_errors` contains entry with `severity == "error" AND source.startswith("lane-")` (per U7b classifier — external warnings excluded); a11y has any violation of severity `critical`; perf payload exceeds 2× the section-type budget; HTML fails semantic-tag check.
...
- **Interaction graph:**
 ...
 - New module trees under `src/clients/`, `src/voice/`, `src/briefs/`, `src/compliance/`, `src/review/`, `src/ads/`, `src/generation/image_composer.py` + `image_layout.py`, **`src/generation/site_render.py` + `src/generation/site_audit.py`** (the two new Phase A shared utilities for site_engine).
 - **New top-level doc directory `docs/rubrics/`** introduced for dual-consumer rubric prose (site-quality.md is its first inhabitant; pattern available for any future rubric shared between lane + skill surfaces).
- **Error propagation:** All shared-infra modules (config loader, persona loader, brief reader, compliance judge, review service, **site renderer, site audit**) fail loud at lane preflight per CLAUDE.md Rule 12. Lane-side errors (fal.ai circuit-broken, R2 upload failure, missing brand asset, **Chromium binary missing, axe-core CLI missing, section payload over budget**) fail the variant immediately rather than degrading silently.
...
- **API surface parity:** **4 new lanes** follow the existing 8-lane substrate contract (LaneSpec + WorkflowSpec + session.md + evaluation-scope.yaml). Promote-findings + render-report pipelines auto-pick up new lanes via `WORKFLOW_SPECS` dict. **Site_engine additionally registers structural-gate functions for the U7b render + U7c audit pre-checks** — same shape as x_engine's slop-check subprocess registration, no substrate contract change.
...
| Pre-publish review SLA breaches due to reviewer unavailability | D14 + TD-9: 25/50/100% SLA nag emails; **auto-pause + operator notification** at 2× SLA (not auto-reject; non-destructive). Paused-N-days operator override CLI (`freddy autoresearch review override`) at ≥ `OPERATOR_OVERRIDE_DAYS` (default 7) covers both-reviewers-unavailable case. |
...
| axe-core CLI not installed (site_engine SE-6 a11y scoring depends on it) | U7c documents the install step (`npm install -g @axe-core/cli`); CI workflow file adds Node setup + install step. site_audit returns `degraded=true` with a clear reason if missing; site_engine treats SE-6 as `n/a` for that variant rather than failing the whole lane. |
...
- **Cross-surface rubric ownership:** `docs/rubrics/site-quality.md` is consumed by both the `site_engine` autoresearch lane AND the `compound-engineering:site-improvement` Claude Code skill (a parallel deliverable). Edits propagate to both; document owner is the autoresearch maintainer; skill consumes the file unchanged.
...
- **site_engine rubric anchors (dual-consumer, lane + skill):** `docs/rubrics/site-quality.md` — SE-1..SE-8 prose, calibrated from 2026-05-13 gofreddy.ai landing-page session
