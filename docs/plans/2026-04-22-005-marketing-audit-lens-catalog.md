...
| Segment-conditional bundles (added 2026-04-22 v2) | 5 bundles × avg ~6 lenses each | ~30 conditional |
| **GRAND TOTAL** | | **~591 lens slots** |

(Of which ~266 fire only when prospect's vertical, geography, or segment is detected. Typical audit fires ~185-195 lenses based on detection signals: 167 always-on + ~18-28 bundle hits.)
...
## Segment-Conditional Bundles (added 2026-04-22 v2)
...
Same architectural pattern as vertical bundles + geo bundles: detection-gated activation, additive on top of always-on.
...
- Resolution channel parity (chat / phone / email / self-serve KB consistency)
- Community-support ratio (forum / Discord / Discourse vs ticket-only)
- Onboarding CRO depth beyond always-on (empty states, progress motivation, multi-channel coordination)
- Paywall sophistication depth beyond always-on (feature gates vs usage limits vs trial expiration vs time-based detail)
...
- Trust-center depth beyond always-on (sub-processor list depth, security whitepaper, SOC 2 Type II vs SOC 3 distinction)
- Internal-comms-as-external-signal (Glassdoor / Comparably review velocity + response rate)
- Account-tier landing pages depth (/enterprise/<industry> sub-pages — beyond core Verticalization)
...
- **Enterprise B2B bundle**: ~6 lenses, fires for ~25% of B2B SaaS (enterprise)
...
All bundle activations are independent and additive. Detection signals don't conflict (vertical = WHAT they sell, geo = WHERE they operate, segment = HOW they sell).
