...
status: DRAFT v3 — Option D surgical edits per spot-check audit (LI-3 CoT collapsed to 3 steps; LI-2 Step 3 softened; LI-3 cold-start spelled out; Component F deferred; per-segment comment-length collapsed; sibling-fork thresholds reframed; substrate-readiness gate added; first-cohort overfitting posture); awaiting pairwise redundancy check + fixture validation
...
revision_history:
  ...
  - 2026-05-19 v3 — Option D surgical edits per spot-check audit (`docs/handoffs/2026-05-19-linkedin-engine-v2-spot-check.md`). LI-3 CoT collapsed from 6 effective sub-steps (1a/1b/1c/1d + Step 2 three-signal + Step 3 verdict) to 3 structured steps per design guide §6 — substantive checks preserved, structure restored. LI-2 Step 3 softened from "identify target audience" to audience-existence-test (verdict 1 if insight non-obvious for AT LEAST ONE primary audience). LI-3 cold-start interaction spelled out explicitly: substrate-provenance check defers to 0.5 + "unknown" when voice.md not provisioned; gestalt AI-slop stack still applies. Component F (capacity-sizing) DEFERRED from v3 production-default to §8 sibling-fork-candidates list with explicit broken-state acknowledgment — Component F either does not ship in the production bundle OR ships with `[CAPACITY-DRAFT]` flag in structural_gate until capacity-model substrate exists or observed-churn signal validates the broken default is harming clients. Per-audience comment-length characterization (founder 30–60w / mid-career IC 50–80w / recruiter 30–50w / industry peer 60–80w) collapsed to single 30–80-word band per Van Der Blom 2025/26 ideal-band reporting — per-audience precision claims removed as over-fit. Sibling-fork raw "3+ clients" thresholds reframed to measurable indicators (e.g., linkedin_carousel triggers when Component C cadence calls for ≥1 carousel/week across ≥3 clients OR carousel-driven engagement accounts for ≥15% of agency-side LinkedIn revenue). §1.5 Substrate-Readiness Gate clause added — Components A–H ship in lockstep with substrate-emission readiness (Component A ships now; B–H ship as their respective extractors / provisioning / target-account-research / etc. substrate catches up). §1 first-cohort overfitting posture added (Polish + US-primary straddle named; substitute-reader list does NOT include Polish aesthetic dermatology or Polish legal services per spot-check finding). All v1 surgical-restoration content preserved (4 score-cap folds in LI-3 / LI-4 + LI-5 hashtag retirement + LI-6 cross-cohort workflow level). 5-criterion ceiling holds. Topic Authority 78% framing in §5 wrapper untouched. Modern-lever CUTS/ADDS lists in §3 untouched. No scope reduction; no new criteria.
---
...
- **Founder / decision-maker** at $1M–$50M ARR scanning for pattern-recognition from someone who solved the same problem; rewards specific numbers and named trade-offs; comments with a specific case
- **Mid-career B2B IC** (engineer, marketer, product, sales) looking for tactical insight to use in tomorrow's meeting; rewards step-by-step framing with concrete reproducible context; comments with implementation detail
- **Recruiter / talent** evaluating culture and trajectory signals; rewards consistent thought leadership over time; comments on cultural fit (typically shorter — recruiter comments trend toward the lower end of the band)
- **Industry peer** wanting a take they can argue with or build on; rewards a contrarian-but-defensible claim; comments with stance-taking (typically longer — peer comments trend toward the upper end of the band)

Substantive comment-seed target: **30–80-word range** (Van Der Blom 2025/26 *Algorithm InSights* ideal band). This is the single literature-supported ideal band — per-audience precision claims (founder 30–60w, mid-career IC 50–80w, recruiter 30–50w, industry peer 60–80w) introduced in v2 are removed in v3 as over-fit; Van Der Blom reports the band as a single number, not segmented by reader type. The judge uses 30–80 words as the comment-substance threshold across all four primary audiences.
...
**Load-bearing assumption named:** the judge tests whether the post would earn a 30–80-word comment from the ~10–20% of readers who comment in that band (the ideal-band reader), NOT from the median reader (whose comments empirically trend much shorter on 2026 LinkedIn). The mechanism families in LI-4 are derived from WikiDisputes / Graham's hierarchy / Welsh playbook reader behavior at the ideal-band reader level. Designing toward the ideal-band reader is defensible; the criterion's anchor is aspirational-but-predictive at that level, not predictive at the median-reader level.
...
**First-cohort overfitting — explicit straddle named (v3).** The spec asserts US-primary at the judge layer and US-primary substitute-reader breadth while shipping against a first cohort that's Polish-primary (Klinika Melitus = Warsaw aesthetic dermatology; DWF Poland = Polish legal services). **The substitute-reader list above does NOT include Polish aesthetic dermatology or Polish legal services.** This is a real reader-vs-fixture mismatch the spec names rather than smooths over. Mitigation posture: Klinika and DWF are documented as first-cohort exceptions; the judge ships US-primary; vertical-specific calibration for Polish aesthetic dermatology and Polish legal services is deferred to per-vertical fixture coverage (see §8.5) rather than baking Polish first-cohort assumptions into the substitute-reader breadth. If client #5+ onboards from another under-represented vertical, the substitute-reader list expands at that point — not pre-emptively. First-cohort overfitting is an explicit risk to monitor across LI-2 / LI-3 / LI-4 / LI-5 anchors (see §8.8).
...
**Substrate-readiness gate (v3).** The Components A–H multi-component LinkedIn program describes the COMPREHENSIVE workflow target. Component A (text post 600–2,000 chars) ships at substrate-current — `session_eval_linkedin_engine.py` reliably emits text posts today. Components B–H ship as substrate emission catches up, NOT as a single bundle:

- **Component B (profile audit + replacement copy)** ships when the profile-shape extractor exists (substrate-side: a callable that pulls structured headline / About / Featured / banner / URL / skills / recommendations from the author's live profile or a local mirror).
- **Component C (content strategy + pillars + cadence + voice substrate)** ships when pillar-extraction substrate + voice.md provisioning substrate exist (substrate-side: a pillar-candidate enumerator + a voice.md populator that materializes `programs/references/voice.md` from prior posts).
- **Component D (comment strategy + 10 named target accounts)** ships when target-account research substrate exists (substrate-side: in-lane creator discovery with follower-band filter + mechanism-family rationale).
- **Component E (DM templates + connection-request strategy)** ships when the DM-template generator exists (substrate-side: persona-anchored variant generator + qualification-arc structurer).
- **Component F (30/60/90 execution roadmap)** **DEFERRED** per item below (Component F currently fails capacity-sizing; see §8 sibling-fork-candidates list).
- **Component G (Topic Authority 12-month compounding plan)** ships when Topic Authority signal-tracking substrate exists (substrate-side: the 360Brew Topic Authority signal poll + month-4–6 inflection detector).
- **Component H (cross-platform syndication rules)** ships when the X-engine handoff schema is locked (substrate-side: X→LI register-translation rule emitter that pairs with the X lane's output schema).

Until each component's substrate emits, `structural_gate` validates Component A only. The spec does not ship known-broken structural_gate layers for components whose substrate is not yet wired; the bundle ships in lockstep with substrate-emission readiness. This matches the "production-grade v1 posture (no demo deadline)" discipline — generalize when value crosses the next 2–3 clients, not pre-emptively against research-derived hypothetical clients.
...
### Component F — 30/60/90 execution roadmap (DEFERRED in v3 — see §8 sibling-fork-candidates list)

**DEFERRED from v3 production-default.** Spot-check audit flagged Component F as knowingly broken with "fix after observed churn" defense — workflow generates 30/60/90 milestones that are calendar-shaped but not capacity-sized to the client; v2 admitted the gap with "remaining capacity-sizing risk flagged for first-cohort empirical re-validation"; that posture violates production-grade-v1 discipline (don't ship known-broken into production-default).

Component F deferred from v3 production-default until: (a) capacity-model substrate is wired in workflow (operator hours/week available, current LinkedIn ramp, prior content cadence as inputs to milestone generation) OR (b) observed-churn signal proves the broken default is harming client outcomes (specifically, first-cohort clients churning around month 3 from calendar-correct-but-capacity-disconnected milestones). Until then, Component F either (i) **does not ship** as part of the production-default bundle, OR (ii) **ships with an explicit `[CAPACITY-DRAFT]` flag** in `structural_gate` that operator-side gates downstream surfacing (the agency reviews the draft roadmap against the actual client's capacity before exposing it; the lane does not auto-ship Component F to clients).

Standard agency deliverable shape (preserved for reference; not production-default in v3):
...
Size envelope (when ungated): ~1,000 words. Includes risk-flags for the patterns that historically derail clients (volume burnout in week 4, no-results-yet anxiety in month 2, scope-expansion temptation in month 3). When Component F ships with `[CAPACITY-DRAFT]` flag, the `structural_gate` validates: 30/60/90 milestones each named explicitly; per-period operator actions enumerated; risk-flag section present; `[CAPACITY-DRAFT]` marker present (production-default bundle excludes Component F entirely until substrate or telemetry resolves the broken state).
...
The v3 production-default bundle is **Components A + B + C + D + E + G + H** — seven components, ~9,000–10,000 words total (Component F's ~1,000 words deferred per §1.5 substrate-readiness gate). The full eight-component shape (with Component F roadmap added back when capacity-model substrate or observed-churn telemetry resolves the broken default) reaches ~10,000–11,000 words, comparable to the marketing-audit lane's envelope. The judge scopes Component A; `structural_gate` validates B + C + D + E + G + H deterministically (Component F gated by substrate-readiness per §1.5). Per the design guide §1.2 (Hard Rules → structural_gate, Principles → judge), the components that have shape-checkable structure (profile field completeness, voice substrate file existence, strategy section presence, comment-target count validity, DM-template variant count, Topic Authority pillar specification, cross-platform rule presence) belong in `structural_gate`. The component that requires semantic reader-effect prediction (Component A — does the post earn dwell + substantive comment + profile-click) belongs in the judge.
...
- **Component F success (DEFERRED in v3):** Component F is not part of the v3 production-default bundle. When eligible to ship (capacity-model substrate wired OR `[CAPACITY-DRAFT]` flag + operator-side gating), success is the client hitting the 30/60/90 milestones on schedule: Day-30 = profile complete + ≥5 posts shipped; Day-60 = full cadence + ≥4 net new ICP-fit connections per week; Day-90 = newsletter launched + first Topic Authority signal visible in 360Brew distribution metrics.
...
- **Component F Goodhart:** workflow generates 30/60/90 milestones that are calendar-shaped but not capacity-sized to the client. **v3:** Component F DEFERRED from production-default (see §1.5 above and §8 sibling-fork-candidates list); not eligible to ship without capacity-model substrate OR `[CAPACITY-DRAFT]` flag + operator-side gating. The "fix after observed churn" defense from v2 is retired.
...
- Step 2: Apply the specificity test — would swapping the author's name or the named entities/numbers produce a different post? Is the insight non-obvious for at least one plausible target reader?
- Step 3: Verify that an inferable target audience exists (the post implies who would use this insight, even if the audience identity is not named explicitly in the post; the audience must be inferable from the substance). The audience does not need to be one specific named segment — verdict 1 if the insight is non-obvious for AT LEAST ONE of the four primary audiences (founder/decision-maker, mid-career B2B IC, recruiter/talent, industry peer). If no inferable audience, emit 0.5 + "unknown" + "cannot infer target audience." Emit verdict + one-sentence justification naming the specific evidence anchor (or its absence) and the inferred audience (or its absence).
...
**Cold-start interaction (v3, spelled out).** In cold-start (`pattern_data_density="cold"`, `voice.md` not provisioned, or `source_data.author_context_known=false`), LI-3's substrate-provenance check on first-person lived-work claims **defers to 0.5 + "unknown" + "voice substrate not provisioned — cannot verify provenance of lived-work claims."** The gestalt AI-slop stack check still applies in cold-start; only the voice-substrate provenance check defers. This prevents systematically penalizing cold-start clients (no prior posts, no populated voice substrate) for naming real customers / colleagues that the judge has no substrate to validate against. When voice substrate is populated with ≥3 author-surface anchors AND `source_data.author_context_known=true`, the substrate-provenance check fires normally and unanchored lived-work specifics score 0.
...
**Required CoT (3 steps per design guide §6):**
- Step 1: Apply the four gestalt / register / cross-platform / substrate checks together and emit per-check findings. (a) Scan for the gestalt AI-tell stack — count how many of (em-dash density / template-phrase opener / symmetrical bullets / P.S.↓ closer / broetry-line density / affective flatness) trigger (deterministic single-tell density gates have already run in `structural_gate`; the judge is scoring the residual). (b) Test for bait-y / Twitter-translated register — does the post read as a contrarian-punch hot-take imported from X, where the rhetorical compression that lands on X (sharp-line aphorism, hostility-shaped opener, ratio-bait framing) lands as bait on the LinkedIn surface? The LinkedIn voice lever is *thoughtful authority*, not contrarian punch. (c) Voice-substrate provenance check on first-person specific lived-work claims — for each named entity (customer, colleague, project, dollar/percentage outcome the author claims to have owned), test whether the entity appears in the loaded voice substrate at `programs/references/voice.md`. Unanchored lived-work specifics are confabulation. **Cold-start handling:** in cold-start (per score-0 prose above), this substrate-provenance sub-check defers to 0.5 + "unknown"; the gestalt and register sub-checks (a) and (b) still apply.
- Step 2: Identify compensating voice markers AND apply the three-signal substance test. For each gestalt signal that triggered in Step 1(a), test whether a compensating voice marker is present (named entity in body, marked affect, internal self-correction, specific anecdote tied to author's professional context). Then apply the three-signal substance test: (i) specificity in the body (not just the opener); (ii) at least one marked affective valence in the body (specific irritation, surprise, regret, delight, stake); (iii) internal-contradiction tolerance (self-correction or acknowledged limit).
...
- **Component F**: DEFERRED in v3 from production-default (see §1.5). Component F either does not ship in the production bundle OR ships with `[CAPACITY-DRAFT]` flag + operator-side gating until capacity-model substrate is wired or observed-churn telemetry validates the broken default is harming clients.
...
When `source_data` lacks prior-author posts (new client, first post, no work-history surface), LI-3 and LI-5 lose their grounding anchor. Component C voice-substrate population at `programs/references/voice.md` and Component B profile rewrite provide the program-level mitigations (the voice substrate is the load-bearing input for LI-3's Step 1(c) substrate-provenance sub-check in the v3 collapsed-CoT structure; the profile rewrite populates the source_data the judge reads in LI-5). **v3 makes the cold-start interaction explicit in the LI-3 score-0 prose and CoT:** when `pattern_data_density="cold"` or `voice.md` is not provisioned, the substrate-provenance sub-check defers to 0.5 + "unknown"; the gestalt AI-slop stack check still applies. For LI-5 cold-start: pass a `source_data.author_context_known = false` flag that triggers LI-5 to emit 0.5 + "unknown" by default rather than scoring blind. Implementation deferred to lane wiring. The `linkedin_engine v040 cold-start mutation` memory provides a structural precedent (`templates/linkedin_engine/skeleton-short_take.md` plus pre-ship checklist).
...
- **Component F (30/60/90 roadmap, DEFERRED in v3, ~40 LOC when eligible):** 30/60/90 milestones each named explicitly; per-period operator actions enumerated; risk-flag section present; `[CAPACITY-DRAFT]` marker present if shipping under draft-flag mode. Not in the v3 production-default bundle until capacity-model substrate wires OR observed-churn telemetry resolves the broken default.
...
The Step 0 research recommends sibling-lane decomposition for the full 2026 LinkedIn surface. Per JR confirmation, sibling-lane forks are deferred to triggered expansion: a sibling lane stands up only when a **measurable indicator** fires (not a raw client count). v3 reframes the v2 "3+ clients" thresholds to measurable indicators per spot-check audit. The triggers:

- **`linkedin_carousel` fork:** trigger when Component C cadence requires ≥1 carousel/week across ≥3 clients OR when carousel-driven engagement accounts for ≥15% of agency-side LinkedIn revenue. **Highest-priority sibling-fork target** per Step 0 research §6 (6.60% engagement, 39% more reach than text). The fork inherits Component A's LI-1..LI-5 *shape* (5 outcome questions) but with carousel-specific criteria likely: trailer-slide-earns-swipe, framework-coherence-across-slides, last-slide-CTA-fit, visual-hierarchy, author-context-coherence. Carousel mechanics differ from text-post mechanics (swipe-completion ≠ scroll-completion), so the criteria are sibling-shaped, not identical-copy.
- **`linkedin_newsletter` fork:** trigger when ≥3 clients launch a newsletter as part of Component G AND ≥1 of them generates documented pipeline contribution from the newsletter surface within a quarter. Newsletter subscribers have separated distribution + different reader (subscribed, expecting depth) + different success conditions (read-through-rate, click-through-to-CTA, save-for-later); criteria likely: opening-earns-the-full-read, single-insight-clarity, voice-residual, subscribe-incentive-coherence, author-context-coherence. Newsletter long-form belongs in this sibling lane, not Component A.
- **`linkedin_live` fork:** trigger when ≥3 clients run ≥1 LinkedIn Live event per quarter AND ≥1 client's Live-event companion content (clips, carousel, newsletter mention) shows ≥2× the engagement of their baseline post cadence. Lower-priority sibling-fork target per Step 0 research (most small-to-medium clients should run 1 Live event per quarter, not weekly). Sibling lane would scope: pre-Live promotion sequence + Live event format + post-Live companion content.
- **`linkedin_profile` fork:** trigger when profile-audit depth grows beyond Component B's current 800-word envelope on ≥2 client deliverables OR when a vertical-specific profile pattern emerges that diverges from the B2B-SaaS-default in Component B (e.g., regulated finance partners need a different About-paragraph register; DTC e-commerce founders need a different Featured-section composition).
- **`linkedin_comment_strategy` fork:** trigger when ≥3 clients have ≥3 overlapping target accounts in their Component D lists AND coordination of who-comments-when becomes an agency operational problem (e.g., to prevent comment-stacking on the same in-lane creator that might trigger LinkedIn's pod-detection algorithm). Until then, Component D inside the single `linkedin_engine` lane is sufficient.

**Sibling-fork posture:** the v2 lane stays single-lane producing the v3 production-default bundle (Components A + B + C + D + E + G + H per §1.5; Component F deferred). Sibling lanes are not pre-emptively built; they fork only when their measurable-indicator trigger fires. This preserves the lane's tractability while preserving optionality for the longer-arc surface expansion.

### Sibling-fork-candidates list (v3 additions)

Two items are flagged on this list in v3, not as sibling lanes proper but as deferred components / lanes pending substrate or telemetry:

- **Component F (capacity-sizing 30/60/90 roadmap)** deferred from v3 production-default until: (a) capacity-model substrate is wired in workflow (operator hours/week available, current LinkedIn ramp, prior content cadence as inputs to milestone generation), OR (b) observed-churn signal proves the broken default is harming client outcomes (first-cohort clients churning around month 3 from calendar-correct-but-capacity-disconnected milestones). Until then, Component F either (i) does not ship in the production-default bundle OR (ii) ships with explicit `[CAPACITY-DRAFT]` flag in structural_gate + operator-side gating (the agency reviews the draft roadmap against the actual client's capacity before exposing it; the lane does not auto-ship Component F to clients). See §1.5 and §6 Component F Goodhart entry.
...
- **LI-1 live "thoughtful authority, not contrarian punch" lever** → preserved in v2 LI-3 score-0 anchor as the "bait-y or Twitter-translated" cross-platform register failure. The lever distinguishes LinkedIn voice from X voice and is load-bearing in LI-3's Step 1(b) CoT (v3 collapsed-CoT structure).
...
- **LI-2 live HARD FLOOR "lived-work claims REQUIRE the named entity in voice.md"** → preserved as a score-0 anchor in LI-3 (voice criterion, where voice substrate is already loaded by `load_source_data`). The original "score capped at 7" semantic is re-expressed as a score-0 condition; the binary system collapses graduated caps to anchor conditions. LI-3 Step 1(c) CoT (v3 collapsed-CoT structure) enforces the substrate provenance check on every first-person specific lived-work claim, with explicit cold-start handling (defers to 0.5 + "unknown" when voice substrate not provisioned). **v2 extension:** Component C's voice substrate validation at `structural_gate` provides the program-level guarantee that the voice.md file exists and is non-empty before any post fixture runs through the judge.
