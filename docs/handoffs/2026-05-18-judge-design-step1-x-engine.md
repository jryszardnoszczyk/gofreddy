...
status: DRAFT v2 — School-B restructure (multi-component bundle deliverable; judge stays atomic at artifact layer; structural_gate validates broader program components)
...
- **C1. Engagement-bait CTAs** — "Like if you agree," "RT if X," "Drop a 🔥," "Comment YES if," "Follow me for more." Grok-flagged as `engagement_arbitrage`; ≈−74× negative-signal cascade per single negative reaction. `structural_gate` literal-string match.
...
- **C13. Brand-account voice on personal account** — "At Acme, we believe in..." Personal accounts get 5-10× more engagement than brand accounts (Conbersa 2026). Fail X-5 voice check. Strategy doc (Component C) defaults to personal-account-first.
...
- **A6. Bookmark-engineering — reference / playbook / checklist content** — §2.1 NEW emphasis. 10× a like. Strategy doc commits 20-30% of content mix to bookmark-shaped artifacts. Lane learns which formats earn the most bookmarks per client across generations.
...
1. **X-6 hook-body alignment promotion — promote only if empirical variance forces (carried over from v1).** The hook-discipline deep research identified a potential 6th criterion targeting the clickbait failure (high impression / low dwell-completion / high negative-signal cascade). v1 + v2 fold hook-body alignment into X-1's Axis C CoT. After redundancy check (§5): if X-1 (3-axis CoT) + X-2 catch ≥90% of seeded clickbait fixtures, fold holds and 5 criteria is correct. If <90%, promote X-6 per the §5 v2.1 documented-exception path — LLM-specific failure surface is documented (xai-org/x-algorithm `engagement_arbitrage` detector + Phoenix bounce-after-hook penalty).
...
6. **Wrapper numerical-weight strip — decision documented (this lane done, REAFFIRMED v2).** v0 named reply/repost/bookmark/dwell/negative numerical weights; v1 stripped; v2 reaffirms strip even though comprehensive scope research surfaces additional operator reconstructions (QT 25×, profile-click 12×, reply-with-author-response 150×). Same Goodhart vector applies. Revisit if the open-source release publishes official numerical weights (currently the repo publishes the formula template + ten-action probability set but NOT coefficients).
