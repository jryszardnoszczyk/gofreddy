...
revision_history:
  ...
  - 2026-05-18 v1-surgical — restored 2 load-bearing items from live code at `fc99d64` per cross-check audit (`docs/handoffs/2026-05-18-judge-design-v1-cross-check.md`): X-2 voice.md HARD FLOOR (named entity REQUIRED in `programs/references/voice.md` for first-person lived-work claims) + X-5 jargon-without-inline-gloss rule (JR accessibility anchor). 2 items intentionally NOT restored at criterion layer: bracket-aware scoring (SHARP/BUILD/CASE-STUDY — flagged §8 open-question 11 to push into structural_gate); X-6 cross-cohort criterion (workflow-level `cross_item_criteria` survives — flagged §8 open-question 12). 5-criterion ceiling preserved.
---
...
**Restoration note (2026-05-18 surgical pass from live code at `fc99d64`).** Cross-check audit (`docs/handoffs/2026-05-18-judge-design-v1-cross-check.md` §X, verdict NEEDS-RESTORATION) flagged four load-bearing items from `autoresearch/archive/v006/workflows/session_eval_x_engine.py` that did not survive v0→v1 synthesis. Surgical restorations applied:

- **X-2 voice.md HARD FLOOR** — restored as explicit score-cap in X-2 Score 0 + new CoT Step 3. Live code: "HARD FLOOR: any first-person specific lived-work claim REQUIRES the named entity to appear in `programs/references/voice.md`." Mechanism: substrate is already loaded as `parents[2]` of `session_dir` via `load_source_data`, so the gate fires judge-side at scoring time, not just at gate time.
- **X-5 jargon-gloss rule** — restored as explicit score-cap in X-5 Score 0 + folded into X-5 CoT Step 2. Live code (live X-1 voice criterion, now v1 X-5): "Jargon without inline plain-English context caps this dimension." JR-iterated accessibility-to-non-engineer-founder/marketer anchor.
- **Bracket-aware scoring (SHARP / BUILD / CASE-STUDY)** — INTENTIONALLY NOT restored as judge criteria; see open-question 11 below. v1's artifact-shape lock (§1.5: single post or thread of 3–12 units) replaces the three-bracket taxonomy at the judge layer. Per-bracket structural prescriptions (SHARP one claim+support pair; BUILD prose intro + structural pivot + 3–5 bullets + authority anchor + outcome metric; CASE-STUDY multi-paragraph narrative + sensory detail + numbers timeline + implication close) belong in `structural_gate` if the workflow still produces bracket-tagged drafts. The bracket field (`length_bracket` in frontmatter, length ranges 250–300 / 500–900 / 1000–1500) remains enforceable at gate level.
- **X-6 cross-cohort criterion** — INTENTIONALLY NOT restored as a 6th judge criterion (preserves the 5-criterion ceiling per design-guide §5). The cross-cohort check survives at workflow level: `SessionEvalSpec.cross_item_criteria={"X-6": CrossItemCriterion(glob="drafts/*.md", max_items=10, words_per_item=400)}` continues to fire across multi-draft sessions. The diversity test ("no two drafts use the same primary differentiator, source, or hook archetype; spread across voice_pillars listed in angle metadata") runs orthogonally to per-draft judging. See open-question 12 below for whether to surface that workflow-level result back into judge scoring.
...
11. **Bracket taxonomy vs locked artifact shape — resolve before spec ships to v006/workflows.** Live code's `structural_gate` enforces a three-bracket taxonomy (SHARP 250–300 chars / BUILD 500–900 / CASE-STUDY 1000–1500) via the `length_bracket` frontmatter field, and live's X-5 criterion encoded per-bracket structural prescriptions (SHARP claim+support pair; BUILD prose intro + structural pivot + 3–5 bullets + authority anchor + outcome metric; CASE-STUDY multi-paragraph narrative + sensory detail + numbers timeline + implication close; pad-to-length ≤4). v1 §1.5 locked artifact-shape to single post (≤280) or thread of 3–12 units — which fits SHARP-as-single-post and BUILD/CASE-STUDY-as-thread, but is not identical to the live taxonomy. Decision needed: (a) keep live's three-bracket structural_gate, treat BUILD and CASE-STUDY as thread sub-shapes the judge sees only as "thread of N units"; (b) collapse to v1's two-shape spec and migrate live's `_LENGTH_BRACKETS` + frontmatter `length_bracket` field accordingly; (c) restore the bracket-aware per-shape prescription as a `structural_gate` check (not a judge criterion — outcome-question-not-feature-check discipline holds). Recommend (c) as least-disruptive: keep judge at outcome questions, push per-bracket structural prescriptions into `structural_gate` where they were always meant to live.

12. **X-6 cross-cohort: workflow-level only, or surface to judge?** Live code wires `cross_item_criteria={"X-6": CrossItemCriterion(glob="drafts/*.md", max_items=10, words_per_item=400)}` in `SessionEvalSpec`. v1 drops X-6 as a judge criterion (preserves 5-criterion ceiling) but the workflow-level diversity check ("no two drafts use the same primary differentiator, source, or hook archetype; spread across voice_pillars") still fires across multi-draft sessions. Decision needed: (a) keep cross_item_criteria active at workflow level only — diversity violations surface as workflow-level telemetry, judge scores each draft independently per X-1..X-5 (current v1 stance, leaner); (b) feed cross_item_criteria output back into the judge as a 0/1 session-level multiplier on top of per-draft scores (forces 6th criterion in spirit if not in count; risks Goodhart on diversity surface markers); (c) promote X-6 to a 6th judge criterion if redundancy-check (§5 v2.1 documented-exception path) shows multi-draft sessions overfitting on same-archetype hooks. Recommend (a) until empirical telemetry forces escalation; same posture as X-6 hook-body alignment in open-question 1.
