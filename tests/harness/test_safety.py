...
def test_check_no_leak_catches_paths_outside_track_scopes(tmp_path):
    """Paths outside every track's allowlist (docs/, tests/, .github/) used to
    be silently ignored by _FIXER_REACHABLE — a premature optimization that
    masked the real bug surfaced in smoke run 20260422-190507 where an agent
    could have written to main repo paths outside its scope. Widened check:
    any new dirty file NOT matching HARNESS_ARTIFACTS is a leak."""
    repo = _init_repo_with_tracked_dirs(tmp_path)
    snapshot = snapshot_dirty(repo)
    (repo / "docs" / "plans" / "new-plan.md").write_text("plan\n", encoding="utf-8")
    leaks = check_no_leak(snapshot, main_repo=repo)
    assert leaks == ["docs/plans/new-plan.md"]


def test_check_no_leak_catches_tests_path_outside_track_scopes(tmp_path):
    """tests/ is the path the widening specifically protects against —
    fixer prompt says 'never modify tests/**' but agents could ignore that."""
    repo = _init_repo_with_tracked_dirs(tmp_path)
...
def test_check_no_leak_still_ignores_harness_artifacts(tmp_path):
    ...
    are generated by the harness itself and must not count as leaks."""
    repo = _init_repo_with_tracked_dirs(tmp_path)
    snapshot = snapshot_dirty(repo)
