...
# ---------------------------------------------------------------------------
# Per-criterion scoring (Stream A axis-collapse fix, graduated 2026-05-13)
# ---------------------------------------------------------------------------
...
def test_batch_critique_unpacks_per_criterion(
    tmp_path, monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_batch_critique_falls_back_to_broadcast_when_per_criterion_absent(
    ...
    """Graceful degradation: if the judge omits ``per_criterion``, the
    bridge falls back to the single-verdict broadcast so older judge
    deployments do not break the sweep."""
    _capture_post(
        monkeypatch,
        _FakeResponse(
            200,
