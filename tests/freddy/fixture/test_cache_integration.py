...
Covers the Phase 8 contract:
  - search pool: cache miss → None (caller live-fetches).
...
def test_sha1_collision_guard_by_arg_equality(tmp_path, monkeypatch):
    ...
    # Search pool → miss returns None, does not leak the other fixture's body.
...
def test_shape_flags_produce_distinct_cache_entries(tmp_path, monkeypatch):
    ...
    # Non-default shape → cache miss (different hash), search pool → None.
