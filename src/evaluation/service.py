...
class EvaluationService:
    ...
    async def evaluate_domain(
        ...
        *,
        ...
        # Combine output text for analysis
        output_text = "\n\n".join(outputs.values())
        ...
        # 5. Length normalization (scale word range by number of output files)
        length_factor = compute_length_factor(domain, output_text, n_items=len(outputs))
    ...
    async def _ensemble_judge_criterion(
        ...
        returns an error DimensionResult with score 0.
        """
        # Build one coroutine per (judge, replicate) pair.
        call_tasks = [
            self._judge_with_deadline(
        ...
        raw = await asyncio.gather(*call_tasks, return_exceptions=True)
