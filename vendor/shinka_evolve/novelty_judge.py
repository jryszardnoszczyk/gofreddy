...
# Gofreddy adaptation: the upstream depends on shinka.{database,llm,prompts}
# which are not installed here. Type stubs preserve the upstream API surface
# while making the file importable in isolation. The LLM tie-breaker path
# is only exercised when ``novelty_llm_client`` is non-None; gofreddy passes
# None for first-pass deployment, so NOVELTY_SYSTEM_MSG / NOVELTY_USER_MSG
# remain unused at runtime. See ``vendor/shinka_evolve/ATTRIBUTION.md`` for
# the full list of modifications relative to the verified upstream SHA.
Program = Any  # type: ignore[misc]
LLMClient = Any  # type: ignore[misc]
NOVELTY_SYSTEM_MSG = ""
NOVELTY_USER_MSG = "{language}\n{existing_code}\n{proposed_code}"
