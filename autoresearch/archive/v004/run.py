...
def is_complete(session_dir: Path) -> bool:
    return runtime_config.is_complete(session_dir)
...
def enforce_iteration_contract(
    ...
    if results_lines_after == results_lines_before:
        print(
            (
                f"WARNING: iteration {iteration} produced no results.jsonl entry. "
...
def run_domain_fresh(domain: str, client: str, context: str, max_iter: int,
    ...
    for i in range(1, max_iter + 1):
        ...
        snapshot_state(session_dir, subdirs)
        ...
        try:
            while True:
                try:
                    ...
                    if current_phase_count > initial_phase_count or is_complete(session_dir) or is_blocked(session_dir):
                        phase_completed = current_phase_count > initial_phase_count or is_complete(session_dir) or is_blocked(session_dir)
        ...
        if (
            ...
            and not is_complete(session_dir)
        ...
        if state_changed(session_dir, subdirs):
            stall_count = 0
        else:
            stall_count += 1
            print(f"No state change. Stall: {stall_count}/{stall_limit}")
...
def run_domain_multiturn(domain: str, client: str, context: str, timeout: int) -> int:
    ...
    state = {
        ...
        "last_snapshot": take_progress_snapshot(session_dir, subdirs),
    ...
    def watchdog_loop():
        while not state["stop"].is_set():
            ...
            if now - state["last_snapshot_at"] >= SNAPSHOT_INTERVAL_SECONDS:
                snapshot = take_progress_snapshot(session_dir, subdirs)
                if snapshot.changed_from(state["last_snapshot"]):
                    state["stall_count"] = 0
                else:
                    state["stall_count"] += 1
    ...
    if exit_code != 0 and not is_complete(session_dir):
