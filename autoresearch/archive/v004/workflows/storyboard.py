...
def completion_guard(eval_summary: dict[str, object]) -> tuple[str | None, str | None]:
    ...
    required = int(os.environ.get("STORYBOARD_COUNT", "5"))
    if len(decisions) < required:
        note = f"Only {len(decisions)} of {required} story plans present; rerun PLAN_STORY to generate all {required} before marking COMPLETE."
        return note, "stories/*.json"
    if any(item.get("decision") != "KEEP" for item in decisions):
