...
Your numeric claims are scored by a **verbatim grounding matcher** that compares each claim in your output against the raw source data files (`pages/*.json`, `competitors/visibility.json`). If you write an aggregate ("65% SOV", "13 of 20 queries", "$449/mo average pricing") the matcher cannot find that aggregate anywhere in the source and your entire output scores as fabricated. **Aggregates are forbidden unless you also quote the underlying source values verbatim.**

- **Bad (aggregate only):** `Semrush dominates pricing queries with 65% SOV across 20 keywords.`
- **Good (aggregate + source quotes):** `Semrush appears in 13 of 20 pricing queries in competitors/visibility.json — example citations "Semrush Business Plan starts at $449.95/mo" (query: "seo tool pricing") and "Semrush Pro at $139.95/mo" (query: "semrush vs ahrefs pricing").`

If you cannot quote the underlying source values, do not make the aggregate claim. Numbers without matchable source quotes = fabricated entities = score 0.
...
- **Page priority order (highest first): pricing > comparison > feature/product > hub/homepage > general.** The pricing page MUST be included in the top 3 if it exists — it is the highest-value commercial query target. A pricing page with thin static content (100-300 words) is still higher priority than a feature page with rich content.
- Run `freddy scrape` on each. Persist to `pages/{slug}.json`.
- **Competitive (rate-limit fast-fail):** Run ONE `freddy visibility --brand "{client}" --keywords "<1-2 inferred pricing keywords>"` call only. If it returns rate-limited or no data within the first result, log `{"type": "competitive", "queries_run": 1, "queries_with_data": 0, "rate_limited": 1, "status": "skipped"}` and move on immediately — do NOT retry with additional keywords. Competitive data is optional; wall time is not.
...
- **Thin/JS-blocked pricing page handling:** If the pricing page has <300 words of static content, DO NOT skip it. Instead, create a structural optimization grounded in the static HTML that WAS captured:
  - Ground claims in: page title, meta description, any plan/tier names visible in static HTML, feature mentions, and any pricing-adjacent text captured.
  - [FAQ] block: answer questions like "What plans does {client} offer?", "How is {client} pricing structured?", "What's included in each plan?" using tier names and feature lists from static HTML. If exact prices aren't in the static HTML, write: "Current pricing is available at the {client} pricing page — plans include [tier names found in HTML]."
  - [HOWTO] block: "How to choose the right {client} plan" — steps based on use case, team size, and feature needs. This answers the pricing query structurally.
  - [FILL] block: comparison of plan tiers using feature differences visible in static HTML (not price figures if unavailable).
  - DO NOT invent specific dollar amounts not present in the scraped content. Mark any pricing figure as `[DYNAMIC: verify at {url}]` if not found in `pages/{slug}.json`.
  - This approach passes grounding (claims traced to static HTML) and partially satisfies GEO-7 (structural pricing guidance answers "how does pricing work" even without exact figures).
...
- **DISCOVER**: Scrape the site, build page inventory. Use `freddy sitemap <url>` first to discover all URLs from the sitemap, then `freddy scrape` on the highest-priority pages. **Limit to MAX_PAGES pages** (read from `$MAX_PAGES` env var, default 6). Pick the highest-priority pages by traffic potential and competitive gap: pricing > comparison > feature/product > hub > general. Persist all scraped PageContent as JSON to `sessions/geo/{client}/pages/{slug}.json`. Downstream stages read from cache instead of re-scraping. If a non-pricing page returns <100 words of text, mark as `status: 'js_blocked'` in results.jsonl and exclude from optimization queue. **Exception: pricing pages are NEVER excluded regardless of word count** — thin pricing pages get structural optimization (see EFFICIENCY MODE thin-content handling).
...
5. **EVALUATE** — Run the session evaluator on the optimized page:
   ...
   ```
   Parse the JSON output. Missing or invalid evaluator output counts as `REWORK`, not pass. Also run `freddy detect --full` on the page URL to get current infrastructure state + DataForSEO + PageSpeed data. Use the Scoring Rubric for after-scoring.
...
10. **Persist state and continue according to strategy.** In fresh mode stop after this phase. In multiturn mode move to the next needed phase.
...
| GEO-7 | Does content directly answer the declared target query? For pricing pages: does content answer "how does pricing work / what plans exist"? | Yes/Partially (explain)/No — fix before proceeding |
...
7. **Page-Type Scoring Patterns** — Average before/after/delta grouped by page type.
...
Write `sessions/geo/{client}/report.json` sidecar with: pages_optimized, avg_delta, kept_count, discarded_count, top_patterns, measurement_queries.
...
{"iteration": N, "type": "optimize", "page": "/path/", "attempt": N,
 "before": {"citability": 0.0, "geo_quality": 0.0, "infra": "pass|fail"},
 "after": {...}, "delta": 0.0, "approach": "...", "status": "kept|discarded"}
// REPORT
{"iteration": N, "type": "report", "pages_optimized": N, "avg_delta": 0.0, "headings_compliant": true, "status": "complete"}
```
