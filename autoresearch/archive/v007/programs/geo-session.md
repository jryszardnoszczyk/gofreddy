...
- **CQ-19:** Before gathering data, check for product-marketing context at exact, narrow paths only: `ls .agents/product-marketing-context.md .claude/product-marketing-context.md 2>/dev/null` from the current working directory. Do **not** run recursive `find` across the repo — wide filesystem traversals trip sandbox safety filters on some backends and cost you the whole iteration. If neither path exists, note it in `findings.md` in one line and proceed; the file is a nice-to-have, not a blocker. Source: Corey Haines `product-marketing-context` convention.
...
Tool failures have three distinct root causes. Diagnose first, then apply the right recovery. **One failed tool call is not a reason to abandon the phase.** Per-fixture scoring is a geometric mean: a single zero from "gave up after the first failure" tanks the whole lane. Always produce at least one structurally-valid `optimized/<slug>.md` plus `gap_allocation.json` — even thin, partially-grounded content scores far above zero.

1. **Sandbox safety filter rejected your command** — error text mentions "flagged for possible cybersecurity risk" or similar. The model's safety system blocked the *shape* of the command, not the intent. Retry with a narrower form: replace recursive `find` with explicit `ls path/to/file`, replace shell pipelines and `--exec` patterns with single read-only commands, replace globs with concrete filenames. Do not abandon the phase; just rephrase.

2. **`freddy` returns `connection_error` / `invalid_url` / API unavailable** — treat as transient on the runtime, not a site outage. Retry once, then fall back to direct fetch:
   - Sitemap: `curl -sL <site>/sitemap.xml` or `curl -sL <site>/robots.txt` (it usually points at sitemaps).
   - Page content: `curl -sL <url>` and extract text/title/h1/h2 with a one-shot Python snippet using `html.parser` or a regex pass; persist to `pages/<slug>.json` with shape `{"url": "...", "final_url": "...", "title": "...", "h1s": [...], "h2s": [...], "text": "..."}` plus a `"method": "fallback_direct_fetch"` marker so downstream logic can see the source.
   - Visibility: if `freddy visibility` fails twice, write `competitors/visibility.json` as `{"method": "fallback_manual", "results": {...}}` populated from cached competitor pages plus any live competitor landing samples you can fetch. Set `CQ-DATA` mode: qualitative positioning, no fabricated citation counts.

3. **Outbound DNS or network is genuinely unreachable** — `curl` itself errors with `Could not resolve host`. You cannot scrape; you also cannot retry your way out. Still produce the deliverable, in this exact order so the structural gate passes: write `gap_allocation.json` with at least one allocation entry derived from the client name + site URL alone; write one `optimized/<slug>.md` containing `[INTRO]` (40–60 words), `[FAQ]` (5+ Q/A), and ≥300 words of content scoped to widely-known public facts about the brand and the target query — and clearly mark every numeric or competitive claim as `[FILL: needs live data]` so it is unmistakably a degraded fallback, not fabricated measured data. This is the *only* situation in which scaffolding from public knowledge is acceptable; anywhere `freddy` or `curl` works, ground in scraped evidence instead.

Log every fallback you take in `findings.md` under `## Disproved` (`[API]` or `[INFRA]`) so the next session knows what failed and what fallback was used.
