...
def summarize(session_dir: str, domain: str, client: str) -> dict:
    ...
    # Count findings via per-lane workflow-spec callback. Prior inline
    # regex dispatch produced 0 counts across all 3 validation lanes
    # because agents had drifted to bullet-list formats. Delegating to
    # the lane keeps format ownership with the workflow.
    findings_file = session_path / "findings.md"
    findings_count = 0
    if findings_file.exists():
        _text = findings_file.read_text()
        findings_count = get_workflow_spec(domain).count_findings(_text)
...
def append_metrics(summary: dict, domain: str, client: str, session_dir: str) -> None:
    """Append one JSONL line to the variant's metrics/ dir. Non-fatal.

    Writes to the variant dir inferred from ``session_dir`` (typically
    ``archive/v006``), not to the runtime mirror (``archive/current_runtime``)
    that the helper script lives under. Before this fix, metrics accumulated
    under ``current_runtime/metrics/`` while evolution consumers read from
    the variant dir — so recent runs were invisible to the reader.
    """
    try:
        session_path = Path(session_dir).resolve()
        try:
            variant_root = session_path.parents[2]
        except IndexError:
