...
Your output is evaluated by LLM judges who check whether findings trace to specific data from your source files (`competitors/*.json`, `_client_baseline.json`). Aggregates and conclusions are valued — but they must be anchored in concrete evidence from the files you read.

- **Bad (vague claim):** `Sketch leads Foreplay with ~35% SOV across design-tool competitors.`
- **Good (claim with traceable evidence):** `Sketch leads Foreplay with ~35% SOV (9 of 26 ads in competitors/sketch.json; example headlines: "Design smarter, not harder", "Made for teams that ship fast", "Professional design tools for modern teams").`
...
1. **CLIENT BASELINE FIRST**: Before gathering competitor data, gather the CLIENT's own ad presence: `freddy search-ads <client_domain>`. Write to `competitors/_client_baseline.json`. Also run `freddy detect <client_domain>` and write to `competitors/_client_baseline_detect.json`. This establishes the reference point for all comparative analysis.
...
#### Bot-Block Detection (scrape analysis)

When `freddy scrape` returns a 200 response, check the result before treating it as real content: if `word_count < 100` AND the text contains any of `update your browser`, `captcha`, `challenge`, or `unsupported`, record `scrape_status: bot_blocked` in the competitor bundle and do NOT infer strategic positioning from the bot-blocked response. Treat the scrape as absent data, not as evidence of a thin competitor site.

#### Known Dead Providers

When profiling a design tool, SaaS, or creative software brand, skip `freddy search-content --platform tiktok` entirely — the TikTok creator search returns 0 results for these verticals as of 2026-04. Pivot straight to Foreplay ads + AI visibility.
...
5. Produce competitive brief with these sections:
   - **Executive Summary** — the "so what?" in 1-2 paragraphs. Open with the single most actionable finding. Lead with a specific competitive gap or opportunity, not a scene-setter. Include a **strategic narrative arc**: "Competitor X is doing A, which means B for the client, creating window C that closes by D." The brief should be organized around a single strategic thesis — every section should serve that argument, not just present findings.
   ...
   - **Changes vs Prior** — delta from prior brief. Follow the **Prior Brief Versioning** rule below to locate the archived prior `brief.md` (NOT `prior_brief_summary.json`, which no longer exists). If a prior brief is found, reference specific metric changes. If first brief, state "Baseline brief — no prior comparison available."
...
8. Append to results.jsonl: `{"iteration": N, "type": "verify", "quality_score": "X/8", "critical_pass": true|false, "weighted_score_normalized_percent": 0-100, "strategist_test": "PASS|FAIL", "attempt": N, "status": "pass|rework|complete"}`. **Note:** `weighted_score_normalized_percent` is a 0-100 float (e.g., 37/40 → 92.5), NOT a raw count. Older sessions used a raw 0-8 count in `weighted_score`; the renamed field prevents ambiguity across runs.
9. **Persist state.**
...
At the start of SYNTHESIZE, locate the most recent archived prior session: run Bash ``ls -t autoresearch/archive/current_runtime/archived_sessions/ 2>/dev/null | grep -- "-competitive-{client}$" | head -1``. If the output is non-empty, use the Read tool on ``{that_dir}/brief.md`` and treat its contents as the prior_brief for real "Changes vs Prior" deltas. If the output is empty, note "Baseline brief — no prior comparison available." Do NOT read `prior_brief_summary.json`; that writer has been removed.
...
Re-read the CONSTITUTION above before writing each section. Additionally: format vacuums need counterfactuals — explain why competitors might NOT use a format before declaring it an opportunity.
