...
def _validate_storyboard_artifacts(session_dir: Path) -> list[str]:
    ...
    if len(source_tuples) > 1 and len(set(source_tuples)) == 1: failures.append("source_analysis_ids_identical_likely_replay")
    return failures
    ...
    try:
        ...
        if rf.exists():
            for line in rf.read_text().splitlines():
                try: last_iter = max(last_iter, int(json.loads(line).get("iteration", 0) or 0))
                except Exception: continue
        with rf.open("a", encoding="utf-8") as f:
            f.write(json.dumps({"type": "structural_gate", "iteration": last_iter, "status": "fail" if gate_failures else "pass", "failures": gate_failures}) + "\n")
        if gate_failures:
            print(
                json.dumps(
                    make_output(
