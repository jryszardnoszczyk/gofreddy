...
def _summarize_visibility(visibility: dict) -> str | None:
    """Compact, judge-readable slice of competitors/visibility.json so
    GEO-2 and GEO-3 can be graded against measured competitive evidence
    instead of relying on artifact prose alone.

    Returns None when the data is missing, errored, or empty so the
    caller can drop the section rather than emit an empty stub.
    """
    if not isinstance(visibility, dict) or "error" in visibility:
        return None
    results = visibility.get("results") if isinstance(visibility.get("results"), dict) else visibility
    if not isinstance(results, dict) or not results:
        return None

    domain_counts: dict[str, int] = {}
    per_query: list[str] = []
    for query, query_data in list(results.items())[:8]:
        if not isinstance(query_data, dict):
            continue
        per_query_top: list[str] = []
        for platform, platform_data in query_data.items():
            if not isinstance(platform_data, dict):
                continue
            for citation in platform_data.get("citations", []) or []:
                ...
                if len(per_query_top) < 3:
                    snippet = (citation.get("snippet") or citation.get("title") or "").strip()
                    if snippet:
                        snippet = snippet[:140]
                    per_query_top.append(f"{platform}:{domain}{f' — {snippet}' if snippet else ''}")
        if per_query_top:
            per_query.append(f"- {query}: " + "; ".join(per_query_top))

    if not domain_counts and not per_query:
        return None

    top_domains = sorted(domain_counts.items(), key=lambda x: (-x[1], x[0]))[:10]
    lines: list[str] = []
    if top_domains:
        lines.append("Top cited domains across measured queries:")
        for domain, count in top_domains:
            lines.append(f"- {domain}: {count}")
    if per_query:
        lines.append("")
        lines.append("Per-query example citations:")
        lines.extend(per_query[:8])
    return "\n".join(lines)
...
def load_source_data(_mode: str, artifact: Path, session_dir: Path) -> str:
    ...
    if visibility_file.exists():
        try:
            visibility_data = json.loads(visibility_file.read_text(encoding="utf-8", errors="replace"))
        except (json.JSONDecodeError, OSError):
            visibility_data = None
        summary = _summarize_visibility(visibility_data) if visibility_data else None
        if summary:
            parts.append(f"## Competitive Citation Evidence (competitors/visibility.json)\n{summary}")
