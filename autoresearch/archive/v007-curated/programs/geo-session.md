...
Prior runs in the archive show `freddy scrape`, `freddy detect`, and `freddy visibility` returning `connection_error` or a missing-CLI error mid-session. Treat these as expected, not blocking:

- If `freddy scrape <url>` fails twice on the same host, switch to direct `urllib`/`requests` extraction and write `pages/{slug}.json` with the same `{url, title, h1s, h2s, text}` shape.
- If `freddy visibility` is unavailable, write `competitors/visibility.json` with `{"error": "<reason>", "results": {}}` and a `landscape_pages.json` sibling built from direct competitor scrapes. Do not invent citation counts; downstream rubrics (CQ-DATA, GEO-3) penalize fabricated measurement.
- If `freddy detect --full` is unavailable, run a `direct_http` audit (robots.txt, sitemap.xml, llms.txt, pricing.md, meta tags, rendered schema) and store under `audits/{slug}.json` with `"method": "direct_http"`.
- Log infra failures as a single `findings.md` entry, then keep building. A degraded-mode session that ships kept artifacts beats a clean-tool session that stalls.

### Non-SaaS Clients (clinical, news, government, educational)

Most CQ items assume a SaaS funnel; some translate poorly to clinical or editorial pages. For these:

- Replace `SoftwareApplication` schema with the right `*WebPage` subtype: `MedicalWebPage` (with `MedicalCondition`, `MedicalAudience`, `signOrSymptom`, `riskFactor`, `possibleComplication`), `NewsArticle`, `GovernmentService`, or `LearningResource`. Keep the multi-type `@graph` pattern.
- Citability moat (GEO-5) shifts from proprietary methodology to first-party authority signals: own clinical volume, named clinicians, peer-reviewed citations, in-network procedures, dated clinical updates.
- `noindex`/`Disallow` rules and AI-bot allowlist (CQ-14) still apply; SaaS-only items (CQ-13 `pricing.md`/`llms.txt`, CQ-16 `SoftwareApplication`) do not — note the omission in `findings.md` rather than forcing them.
- Honest competitive positioning (GEO-3) compares against peer institutions (Cleveland Clinic, AHA, CDC, NIH, Reuters, AP, etc.), not direct competitors in the SaaS sense.

### Comparison Table Hygiene

When `competitors/visibility.json` has `"error"` or empty `results`, the comparison table must NOT include a "Citation count" column padded with "Not measured" — a column of N/A rows zeros GEO-2 (it reads as filler). Drop the column entirely and add a single sentence above the table noting that citation counts are unmeasured because `freddy visibility` was unavailable.
