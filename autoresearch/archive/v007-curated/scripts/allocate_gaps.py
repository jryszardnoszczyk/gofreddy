...
def extract_competitor_gaps(competitive_data: dict) -> list[str]:
    """Rank candidate competitor angles by measured citation frequency.

    Earlier revisions returned an alphabetically sorted set, which made
    angle assignment effectively random and frequently picked low-signal
    domains (cdn hosts, archive.org). Sorting by citation count first
    surfaces the actual competitor a page should differentiate against;
    alphabetical is only the tiebreaker.
    """
    counts: dict[str, int] = {}

    def _walk(node: object) -> None:
        if isinstance(node, dict):
            citations = node.get("citations") if isinstance(node.get("citations"), list) else None
    ...
    if isinstance(competitive_data, dict):
        _walk(competitive_data)

    return [domain for domain, _ in sorted(counts.items(), key=lambda x: (-x[1], x[0]))]
