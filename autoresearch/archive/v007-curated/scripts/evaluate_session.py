...
def _artifact_hash(artifact: Path) -> str | None:
    """SHA256 of artifact content; None on read failure (fall through to eval)."""
    try:
...
def _load_dedup_cache(session_dir: Path) -> dict:
    path = session_dir / _DEDUP_CACHE_NAME
    if not path.exists():
        return {}
    try:
...
def _store_dedup_cache(session_dir: Path, cache: dict) -> None:
    try:
        (session_dir / _DEDUP_CACHE_NAME).write_text(
...
async def evaluate_all_criteria(
    ...
    failed_count = compute_weighted_failure_count(failed)
...
def _validate_storyboard_artifacts(session_dir: Path) -> list[str]:
    import os
    failures = []
    sd, sbd, patd = session_dir / "stories", session_dir / "storyboards", session_dir / "patterns"
    for p in (sorted(sd.glob("*.json")) if sd.exists() else []):
        try: d = json.loads(p.read_text(encoding="utf-8", errors="replace"))
        except Exception: continue
        if len(d.get("scenes", [])) < 2: failures.append(f"{p.name}: <2 scenes")
    pids = set()
    for p in (patd.glob("*.json") if patd.exists() else []):
        try: pdata = json.loads(p.read_text(encoding="utf-8", errors="replace"))
        except Exception: continue
        if "_analysis_id" in pdata: pids.add(pdata["_analysis_id"])
    source_tuples = []
    for sb in (sorted(sbd.glob("*.json")) if sbd.exists() else []):
        try: d = json.loads(sb.read_text(encoding="utf-8", errors="replace"))
        except Exception: continue
        if os.environ.get("TASK_CLIENT_MODE") != "mock" and d.get("render_is_stale"): failures.append(f"{sb.name}: render_is_stale=True")
        sids = tuple(d.get("source_analysis_ids", []))
        source_tuples.append(sids)
        if pids and sids and not set(sids).issubset(pids): failures.append(f"{sb.name}: source_analysis_ids not in patterns/")
    return failures


def main() -> None:
    parser = argparse.ArgumentParser(description="Session evaluator — per-criterion LLM-based quality feedback.")
    parser.add_argument("--domain", required=True, choices=["geo", "competitive", "monitoring", "storyboard", "x_engine", "linkedin_engine"], help="Domain to evaluate")
    ...
    # Artifact-hash dedup: if this artifact's content hasn't changed since
    # the prior evaluation, replay cached stdout. Prevents the 24 structural_gate
    # flood we saw on monitoring/Shopify (agent re-ran evaluate_session.py
    # per artifact per iteration even though artifacts hadn't changed).
    cache_key = f"{args.domain}:{args.mode}:{artifact}"
    cur_hash = _artifact_hash(artifact)
    if cur_hash is not None:
        cache = _load_dedup_cache(session_dir)
        prior = cache.get(cache_key)
        if isinstance(prior, dict) and prior.get("hash") == cur_hash and "stdout" in prior:
            # Cached hit: replay prior stdout, skip gate + LLM criteria.
            sys.stdout.write(prior["stdout"])
            if not prior["stdout"].endswith("\n"):
                sys.stdout.write("\n")
            return

    def _cache_store(produced: str) -> None:
        if cur_hash is None:
            return
        cache = _load_dedup_cache(session_dir)
        cache[cache_key] = {"hash": cur_hash, "stdout": produced}
        _store_dedup_cache(session_dir, cache)

    try:
        gate_failures: list[str] = []
        if args.domain == "storyboard":
            gate_failures.extend(_validate_storyboard_artifacts(session_dir))
        gate_failures.extend(run_structural_gate(args.domain, args.mode, artifact, session_dir))
        # Only write structural_gate to results.jsonl on FAIL. Pass is a
        # non-event (no diagnostic value, clutters the phase ledger). U4
        # normalizer in run.py injects correct iteration from session state —
        # don't hardcode from argv here.
        if gate_failures:
            rf = session_dir / "results.jsonl"
            with rf.open("a", encoding="utf-8") as f:
                f.write(json.dumps({"type": "structural_gate", "status": "fail", "failures": gate_failures}) + "\n")
        if gate_failures:
            produced = json.dumps(
                make_output(
                    decision="DISCARD",
            ...
            print(produced)
            _cache_store(produced)
            return

        produced = json.dumps(asyncio.run(evaluate_all_criteria(args.domain, args.mode, artifact, session_dir)), indent=2)
        print(produced)
        _cache_store(produced)
