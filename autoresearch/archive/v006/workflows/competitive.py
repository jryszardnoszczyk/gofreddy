...
def count_findings(text: str) -> int:
    """Count competitive findings. Agents mix bullets (prose) + markdown
    tables (competitor archetypes). Counts whichever is richer: bullets
    under `## Cross-Cutting Patterns` etc. OR table rows under
    `## Named Taxonomy`."""
    bullets = len(re.findall(r"^- ", text, re.MULTILINE))
    table_rows = len(re.findall(r"^\| [A-Za-z]", text, re.MULTILINE))
