...
try:  # watchdog lives under scripts/; callers put scripts/ on sys.path
    ...
    import sys as _sys
    print(
        f"runtime.config: watchdog import failed ({_watchdog_exc}) — phase ledger check disabled",
        file=_sys.stderr,
    )
...
# (structural validator #4/#10 explicitly accept digest-only output), and
# storyboard phases can be backend-blocked. Phase-ledger enforcement is
# therefore scoped to competitive — the only lane where Root Cause B
# (agent self-declaring COMPLETE after `synthesize` without running
# `verify`) was empirically observed.
_LEDGER_ENFORCED_DOMAINS: set[str] = {"competitive"}
...
def _status_declared(session_dir: Path, status_token: str) -> bool:
    """Match a `## Status: <token>` declaration only at the start of a line.

    Substring matching falsely matched the phrase when it appeared inside
    agent prompts or instructions (e.g. "set `## Status: COMPLETE` only
    after report creation"). A real status declaration is always at line
    start; anchoring there eliminates the false positive.
    """
...
def _all_phases_logged(session_dir: Path, domain: str) -> bool:
    """True iff every phase in ``TRACKED_PHASE_TYPES[domain]`` has a successful
    entry in ``results.jsonl``.

    Competitive's failure mode: agent writes `## Status: COMPLETE` after the
    `synthesize` phase without running `verify`. Line-anchored status match
    (U12) catches the instruction-text false positive but cannot tell whether
    the agent actually finished all phases. This ledger check does.
    """
    required = TRACKED_PHASE_TYPES.get(domain)
    if not required:
        return True
...
def is_complete(session_dir: Path, domain: str | None = None) -> bool:
    if not _status_declared(session_dir, "COMPLETE"):
        return False
    if domain not in _LEDGER_ENFORCED_DOMAINS:
        return True
    return _all_phases_logged(session_dir, domain)


def is_blocked(session_dir: Path) -> bool:
    return _status_declared(session_dir, "BLOCKED")
...
- Complete exactly ONE phase, persist state to files, then EXIT THE SUBPROCESS.
- Do NOT write `## Status: COMPLETE` until every phase listed under `## Completion` has a corresponding entry in `results.jsonl`.
- Do NOT continue automatically into the next phase after phase completion — the harness spawns the next subprocess.
- Your conversation state will not be preserved after this process exits. Files remain the only state that survives.
