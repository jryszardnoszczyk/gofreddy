...
- **Cadence classification before volume.** Use `started_at` on ads to classify deployment as burst / sustain / drip-burst / dump-and-coast before reporting ad volume. Raw counts without cadence are misleading (a 40-ad burst and 40-ad sustain mean different things). Cross-check bursts against launch-phase signals (Product Hunt, waitlist, Beta sticker) — a burst coinciding with GTM launch reads differently from performance testing.
- **Run every competitor through the 15-objection gap taxonomy.** Search each competitor's site for their response to 15 standard buyer objections (too expensive, already use X, security concerns, can it scale, etc.). Absences are positioning holes, not just missing content. Clustered absences point to structural motion mismatches the client can exploit. See `programs/references/competitor-teardown-and-pricing.md`. Source: Corey Haines `sales-enablement`.
- **Tag every competitor page / ad by buyer type** — Technical, Economic, or Champion. Homepage for Technical but ads for Champion = multi-persona motion OR misalignment; both are findings that strengthen CI-3 (competitor trajectory).
- **Pricing-page teardown layer.** For each competitor with a pricing page, record (a) value metric (per-seat / per-usage / per-feature / etc.), (b) tier count (2 = SMB/Ent split, 3 = Good-Better-Best anchor, 4+ = proliferation signal), (c) psychology signals (anchoring, decoy middle tier, charm vs round pricing). A competitor with inconsistent tier prices across channels is likely running WTP research — predictive signal for upcoming price change.
- **Buyer-stage coverage audit.** Classify competitor content by buyer stage (Awareness / Consideration / Decision / Implementation) via query-pattern matching. A competitor saturating Consideration but absent on Implementation is leaving retention to the client to win — asymmetric opportunity for CI-5. Source: Corey Haines `content-strategy`.
- **Cost-of-delay framing is required on every CI-4 recommendation.** A rec without "what the client loses each {week|month|sprint} they don't act" is a "consider this" — with one, it's a "start this before N." Templates: revenue cost, competitive cost, compliance cost, talent cost. Source: Corey Haines `sales-enablement`.
- **Preflight check.** Before gathering data, check for `.agents/product-marketing-context.md` (or `.claude/product-marketing-context.md`). If present, read it first — it captures the client's ICP, direct/secondary/indirect competitors, top objections, JTBD Four Forces, verbatim customer language, brand voice. If absent, note it in `findings.md` and proceed with scraped evidence. Source: Corey Haines `product-marketing-context`.
- **Prose hygiene pass on brief.** Before committing `brief.md`, strip AI-tell vocabulary (`utilize/leverage/facilitate/robust/comprehensive/pivotal`), filler intensifiers (`absolutely/actually/clearly/very/just`), and over-used transitions (`that being said`, `it's worth noting`, `at its core`). Em-dash heuristic: >1 per page = rewrite. See `programs/references/prose-hygiene.md`.
