...
6. If low-volume (< ~30 mentions), follow these sub-steps IN ORDER (Step 5 MUST already have happened — do not re-order):
   ...
   - b. **MANDATORY** — Run the real persist command:
     ...
     ```
     Verify the response contains a persisted digest id. **Calling `freddy digest persist --help` is NOT a substitute — the runner detects help-only invocations and will warn, and a subsequent week's LOAD_CONTEXT will miss this week.** If the command errors, record the error in session.md; a persist error is non-blocking and does not prevent COMPLETE.
   - c. Set `## Status: COMPLETE` in session.md after steps a–b (persist errors are non-blocking).
...
1. `findings.md` has at least 1 entry under "## Confirmed" with evidence from this session. For low-volume sessions (< 30 mentions), a data-availability observation counts (e.g., "0 mentions returned — possible monitor config or pipeline gap").
2. `digest.md` exists. For full-volume sessions (≥ 30 mentions) also verify `recommendations/executive_summary.md` and `recommendations/action_items.md`.
3. `results.jsonl` has at least 1 entry per completed phase

If findings.md is empty on a full-volume session, you are NOT done. Write at least your top 3 observations before completing.
