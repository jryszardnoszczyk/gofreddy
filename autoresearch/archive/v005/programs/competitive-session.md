...
**Trimming rule**: When trimming brief.md to meet the 2000-word limit, cut prose and filler — never remove verbatim source quotes. A brief that passes word count but has no source quotes fails the outer grounding check and scores 0. Write at 1500-1800 words from the start; do not write 4000+ words and trim 65%.
...
5. **Word budget**: Write brief.md targeting 1500-1800 words total. Build tight from the start — do NOT write a 4000+ word draft and then trim. Rough budget: executive summary 200w, SOV section 100w, per-competitor section 200w each, recommendations 300w, known unknowns + monitoring triggers 200w. Include ≥2 verbatim quoted ad headlines per competitor with ad data; ≥1 verbatim website quote for scrape-only competitors.

6. Produce competitive brief with these sections:
   ...
   - **Monitoring Triggers** — specific thresholds per recommendation:
     ...
     ```
6. Write to `brief.md`
8. Update session.md: synthesized=yes
9. Append to results.jsonl: `{"iteration": N, "type": "synthesize", "sections": N, "taxonomies_built": N, "status": "done"}`
...
1. Read `brief.md`
1a. **GROUNDING GUARD** — before running the evaluator, scan brief.md for verbatim source quotes. For each competitor that has ads, at least one ad headline must appear verbatim (in `"double quotes"`) in brief.md. For scrape-only competitors, at least one verbatim website copy phrase must appear. If any competitor lacks verbatim evidence, add the missing quotes now — do not proceed to evaluation without them. The outer grounding check assigns score=0 when no source quotes are present, regardless of evaluator decision.
...
6. brief.md contains verbatim source quotes: ≥2 ad headlines per competitor with ad data, ≥1 website copy phrase per scrape-only competitor — verified by reading brief.md against competitors/*.json, not assumed
